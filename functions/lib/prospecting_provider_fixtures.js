...
exports.FixtureProvider = exports.PERSON_FIXTURES = exports.COMPANY_FIXTURES = exports.NotConfiguredProvider = void 0;
exports.fixtureSeniority = fixtureSeniority;
...
exports.NotConfiguredProvider = NotConfiguredProvider;
/**
...
class FixtureProvider {
    constructor(options = {}) {
        ...
        this.companies = options.companies ?? exports.COMPANY_FIXTURES;
        this.people = options.people ?? exports.PERSON_FIXTURES;
    }
    isConfigured() {
        return true;
    }
    ...
    toCompany(fixture) {
        return (0, prospecting_providers_js_1.assertCompanySane)({
            ...fixture,
    ...
    async searchBusinesses(criteria) {
        ...
        return page.map((fixture, index) => ({
            ...
            cursor: index === page.length - 1 && nextStart < matches.length
                ? String(nextStart)
                : null,
        }));
    }
    async enrichCompany(input) {
        ...
        const fixture = this.companies.find((entry) => entry.provider_org_id === input.providerOrgId) ??
            this.companies.find((entry) => entry.name.toLowerCase() === input.name.toLowerCase());
        ...
        return {
            ...
            fieldsDiscovered: ['employee_count', 'linkedin_url', 'industry', 'address'].filter((field) => fixture[field] !== null),
        };
    }
    async findDecisionMakers(input) {
        this.guard('FIND_DECISION_MAKERS');
        const matches = this.people.filter((person) => person.orgId === input.providerOrgId);
        ...
        return matches.slice(0, Math.max(1, input.limit)).map((person) => ({
            person: (0, prospecting_providers_js_1.assertNoFabrication)({ ...stripFixtureFields(person) }, this.key, 'FIND_DECISION_MAKERS'),
        }));
    }
    async enrichPerson(input) {
        this.guard('ENRICH_PERSON');
        const fixture = this.people.find((person) => person.provider_person_id === input.providerPersonId);
        if (fixture === undefined || this.options.empty === true) {
            throw new prospecting_providers_js_1.ProviderError({
                code: 'NO_RESULT',
                provider: this.key,
        ...
        return {
            ...
            fieldsDiscovered: ['email', 'phone', 'linkedin_url', 'job_title'].filter((field) => fixture[field] !== null),
        };
    }
    async researchCompany(input) {
        this.guard('RESEARCH_COMPANY');
        const fixture = this.companies.find((entry) => entry.name.toLowerCase() === input.name.toLowerCase());
        const research = fixture !== undefined ? RESEARCH_FIXTURES[fixture.provider_org_id ?? ''] : undefined;
        const profiles = fixture === undefined
            ? []
            : [fixture.instagram_url, fixture.facebook_url, fixture.linkedin_url].filter((value) => value !== null);
        return {
...
function stripFixtureFields(fixture) {
    const { orgId: _orgId, ...person } = fixture;
    return person;
}
/** Seniority mapping is exercised here too, so the table cannot rot unused. */
function fixtureSeniority(title) {
    return (0, prospecting_providers_js_1.seniorityFromTitle)(title);
}
