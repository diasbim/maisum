...
exports.ApolloProvider = exports.APOLLO_SENIORITIES = exports.APOLLO_KEY = void 0;
exports.employeeRangeParam = employeeRangeParam;
exports.confidenceFromMatch = confidenceFromMatch;
exports.emailStatusFrom = emailStatusFrom;
exports.industryFrom = industryFrom;
exports.statusToCode = statusToCode;
exports.seniorityFrom = seniorityFrom;
 ...
 */
function employeeRangeParam(min, max) {
    if (min === null && max === null)
        return null;
    return `${Math.max(1, min ?? 1)},${max ?? 10000}`;
}
/**
...
function confidenceFromMatch(value) {
 ...
 */
function emailStatusFrom(value, hasEmail) {
    if (!hasEmail)
        return 'UNKNOWN';
    if (typeof value !== 'string')
        return 'UNVERIFIED';
 ...
 */
function industryFrom(industry, keywords, name) {
    const haystack = [
...
class ApolloProvider {
    ...
    async searchBusinesses(criteria) {
        ...
        const keywords = criteria.industries.flatMap((businessType) => {
            const entry = prospecting_config_js_1.ICP_INDUSTRIES.find((icp) => icp.businessType === businessType);
    ...
    toCompany(organization) {
        ...
        const record = {
            ...
            phone: asStringField(organization.sanitized_phone) ??
    ...
    async enrichCompany(input) {
        ...
        else if (input.linkedinUrl !== null)
            query.linkedin_url = input.linkedinUrl;
        else
        ...
        const industry = industryFrom(organization.industry, organization.keywords, asStringField(organization.name) ?? input.name);
        if (industry !== null)
    ...
    async findDecisionMakers(input) {
        const body = await this.request('FIND_DECISION_MAKERS', '/mixed_people/api_search', {
            ...
            body: {
                ...
                person_seniorities: [...exports.APOLLO_SENIORITIES],
                organization_ids: input.providerOrgId === null ? undefined : [input.providerOrgId],
                q_organization_domains_list: input.domain === null ? undefined : [input.domain],
                page: 1,
        ...
        return people.flatMap((raw) => {
            ...
            try {
                return [{ person: (0, prospecting_providers_js_1.assertNoFabrication)(record, this.key, 'FIND_DECISION_MAKERS') }];
            }
            catch {
                return [];
            }
    ...
    async enrichPerson(input) {
        ...
        if (title !== null)
            mapped.seniority = (0, prospecting_providers_js_1.seniorityFromTitle)(title);
 ...
 */
function seniorityFrom(raw, title) {
