...
app.post('/customer/nfc-cards/link', async (req, res) => {
    try {
        const result = await handleCustomerNfcCardLinkRequest(req, requireBodyObject(req.body));
        return res.json({ success: true, data: result });
    }
    catch (error) {
        return respondCustomerCoreError(res, error);
...
app.post('/customer/nfc-cards/revoke', async (req, res) => {
    try {
        const result = await handleCustomerNfcCardRevokeRequest(req, requireBodyObject(req.body));
        return res.json({ success: true, data: result });
    }
    catch (error) {
        return respondCustomerCoreError(res, error);
...
app.post('/merchant/customer-nfc/link', async (req, res) => {
    try {
        const result = await handleMerchantNfcCardLinkRequest(req, requireBodyObject(req.body));
        return res.json({ success: true, data: result });
    }
    catch (error) {
        return respondCustomerCoreError(res, error);
...
app.post('/merchant/customer-nfc/resolve', async (req, res) => {
    try {
        const result = await handleMerchantNfcCardResolveRequest(req, requireBodyObject(req.body));
        return res.json({ success: true, data: result });
    }
    catch (error) {
        return respondCustomerCoreError(res, error);
...
app.post('/merchant/customer-nfc/revoke', async (req, res) => {
    try {
        const result = await handleMerchantNfcCardRevokeRequest(req, requireBodyObject(req.body));
        return res.json({ success: true, data: result });
    }
    catch (error) {
        return respondCustomerCoreError(res, error);
        ...
        switch (entityType) {
            case 'customer':
                ...
                if (Object.prototype.hasOwnProperty.call(payload, 'archived_at') ||
                    ...
                    const result = await archiveCustomerViaSync(authedReq, payload, entityId);
                    return res.json({ success: true, data: result });
                }
...
function nfcCardLinkFromSnapshot(cardUid, snapshot) {
    if (!snapshot.exists)
        return null;
    const data = snapshotDataRecord(snapshot);
...
async function findActiveNfcCardLink(cardUid) {
...
async function linkNfcCardToCanonicalCustomer(options) {
    ...
    return admin.firestore().runTransaction(async (transaction) => {
        ...
        if (existing != null &&
            ...
            throw new CustomerCoreError(409, 'nfc_card_already_linked', 'This NFC card is already linked to a different customer.', { card_uid_last4: last4(cardUid) });
        }
...
async function revokeNfcCardLink(options) {
    ...
    await admin.firestore().runTransaction(async (transaction) => {
        ...
        if (!existing || existing.status !== 'ACTIVE') {
            throw new CustomerCoreError(404, 'nfc_card_not_found', 'NFC card is not currently linked.');
        }
        if (options.expectedCanonicalCustomerId != null &&
...
async function resolveBusinessRelationshipForCanonicalId(merchantId, canonicalCustomerId) {
    try {
        return await requireCustomerBusinessRelationship({ firebaseUid: '', canonicalCustomerId, accountData: {} }, merchantId);
    }
    catch (error) {
...
async function requireMerchantRequestAccess(req) {
    if (!req.merchantId || !(await requestCanAccessMerchant(req, req.merchantId))) {
        throw new CustomerCoreError(403, 'merchant_access_denied', 'Merchant access is required.');
    }
    return req.merchantId;
}
async function handleCustomerNfcCardLinkRequest(req, payload) {
    requireCustomerFeature('customerNfcEnabled');
    const account = await requireBoundCustomerAccount(req);
    const cardUid = (0, customer_nfc_js_1.normalizeNfcCardUid)(requirePayloadString(payload, 'card_uid'));
    ...
    (0, customer_nfc_js_1.logCustomerNfcEvent)({
...
async function handleCustomerNfcCardRevokeRequest(req, payload) {
    requireCustomerFeature('customerNfcEnabled');
    const account = await requireBoundCustomerAccount(req);
    const cardUid = (0, customer_nfc_js_1.normalizeNfcCardUid)(requirePayloadString(payload, 'card_uid'));
    ...
    (0, customer_nfc_js_1.logCustomerNfcEvent)({ event: 'revoked', surface: 'customer', cardUidLast4: last4(cardUid) });
    return { card_uid: cardUid, revoked: true };
}
async function handleMerchantNfcCardLinkRequest(req, payload) {
    requireCustomerFeature('customerNfcEnabled');
    ...
    const rawPhone = maybePayloadString(payload, 'phone');
    const customerName = maybePayloadString(payload, 'customer_name', 'customerName', 'name');
    const createCustomerIfMissing = maybePayloadBoolean(payload, 'create_customer_if_missing', 'createCustomerIfMissing') ?? true;
...
async function handleMerchantNfcCardResolveRequest(req, payload) {
    requireCustomerFeature('customerNfcEnabled');
    const merchantId = await requireMerchantRequestAccess(req);
    const cardUid = (0, customer_nfc_js_1.normalizeNfcCardUid)(requirePayloadString(payload, 'card_uid'));
    const createCustomerIfMissing = maybePayloadBoolean(payload, 'create_customer_if_missing', 'createCustomerIfMissing') ?? true;
    ...
    if (!cardLink) {
        ...
        throw new CustomerCoreError(404, 'nfc_card_not_found', 'NFC card is not linked to any customer.');
    }
    let relationship = await resolveBusinessRelationshipForCanonicalId(merchantId, cardLink.canonicalCustomerId);
    let customerCreated = false;
    if (!relationship) {
        if (!createCustomerIfMissing) {
            throw new CustomerCoreError(404, 'customer_business_not_found', 'Business is not linked to this customer yet.');
        }
        const identitySnapshot = await canonicalCustomerIdentityRef(cardLink.canonicalCustomerId).get();
        const phoneE164 = maybePayloadString(snapshotDataRecord(identitySnapshot), 'phone_e164');
        if (!phoneE164) {
            throw new CustomerCoreError(409, 'customer_identity_missing_phone', 'Customer identity has no phone number to bootstrap a new business customer.');
        }
        await linkCanonicalCustomerToBusinessCustomer({
            merchantId,
            customerId: null,
        ...
        relationship = await resolveBusinessRelationshipForCanonicalId(merchantId, cardLink.canonicalCustomerId);
        if (!relationship) {
            throw new CustomerCoreError(500, 'customer_core_resolution_failed', 'Unable to resolve business customer after creation.');
        }
    }
    (0, customer_nfc_js_1.logCustomerNfcEvent)({
        event: 'resolved',
...
async function handleMerchantNfcCardRevokeRequest(req, payload) {
    requireCustomerFeature('customerNfcEnabled');
    const merchantId = await requireMerchantRequestAccess(req);
    const cardUid = (0, customer_nfc_js_1.normalizeNfcCardUid)(requirePayloadString(payload, 'card_uid'));
    const cardLink = await findActiveNfcCardLink(cardUid);
    ...
    const relationship = await resolveBusinessRelationshipForCanonicalId(merchantId, cardLink.canonicalCustomerId);
    if (!relationship) {
        throw new CustomerCoreError(403, 'merchant_nfc_card_access_denied', 'This card does not belong to a customer of your business.');
    }
...
async function resolveCanonicalCustomerIdForNfcBackfillItem(phoneE164, merchantId, customerName, dryRun) {
    if (dryRun) {
 ...
 */
async function handleNfcCardBackfillRequest(body) {
    const payload = requireBodyObject(body);
    const dryRun = maybePayloadBoolean(payload, 'dry_run', 'dryRun') ?? false;
    const rawItems = payload.items;
    if (!Array.isArray(rawItems) || rawItems.length === 0) {
        throw new CustomerCoreError(400, 'nfc_backfill_items_required', 'Provide a non-empty items array.');
    }
    if (rawItems.length > 200) {
        throw new CustomerCoreError(400, 'nfc_backfill_batch_too_large', 'Provide at most 200 items per request.');
    }
    const results = [];
    let linkedCount = 0;
    let skippedCount = 0;
    for (const rawItem of rawItems) {
        ...
        const item = rawItem;
        try {
            const cardUid = (0, customer_nfc_js_1.normalizeNfcCardUid)(requirePayloadString(item, 'card_uid'));
            ...
            const canonicalCustomerId = await resolveCanonicalCustomerIdForNfcBackfillItem(phoneE164, merchantId, customerName, dryRun);
            if (dryRun) {
                const existingCardLink = await findActiveNfcCardLink(cardUid);
                results.push({
                    card_uid: cardUid,
                    status: existingCardLink && existingCardLink.canonicalCustomerId === canonicalCustomerId
                        ? 'already_linked'
                        : 'dry_run',
                    canonical_customer_id: canonicalCustomerId,
                });
...
async function archiveCustomerViaSync(req, payload, customerId) {
    const merchantId = req.merchantId;
    const actorAppUserId = requireAuthenticatedAppUserId(req);
    let archiveMutation;
    try {
        archiveMutation = (0, sync_backend_js_1.resolveCustomerArchiveMutation)(payload, actorAppUserId);
    }
    catch (error) {
    ...
    if (!archiveMutation.touched) {
        throw new CustomerCoreError(400, 'customer_archive_missing', 'Customer archive mutation is missing archived_at.');
    }
    const updatedAt = pickNumber(payload, 'updated_at') ??
    ...
    await admin.firestore().runTransaction(async (transaction) => {
        ...
        if (tombstoneSnapshot.exists) {
            ...
            throw new CustomerCoreError(409, 'customer_deleted_tombstone_conflict', 'Customer was permanently deleted and cannot be restored or updated.', {
                merchant_id: merchantId,
                customer_id: customerId,
        ...
        transaction.set(customerRef, {
            ...customerPatch,
            ...(0, sync_backend_js_1.buildCustomerArchiveFirestorePatch)(archiveMutation, updatedAt),
        }, { merge: true });
    });
    return {
        merchant_id: merchantId,
...
async function cancelSaleViaSync(req, payload, saleId) {
    ...
    return {
        ...
        cancelled_at: pickNumber(loyaltyResult, 'cancelled_at') ??
            cancellationRequest.cancelledAt,
        cancelled_by_app_user_id: maybePayloadString(loyaltyResult, 'cancelled_by_app_user_id') ??
            cancellationRequest.cancelledByAppUserId,
        cancellation_reason: maybePayloadString(loyaltyResult, 'cancellation_reason') ??
            cancellationRequest.cancellationReason,
        replacement_sale_id: maybePayloadString(loyaltyResult, 'replacement_sale_id') ??
...
async function applySaleCancellationToLoyaltyState(params) {
    ...
    const result = await admin.firestore().runTransaction(async (transaction) => {
        ...
        if (saleLedgerSnapshot.exists) {
            ...
            const expectedReversalEntry = (0, sync_backend_js_1.buildSaleReversalLoyaltyLedgerEntry)({
            ...
            const reversalEntry = reversalLedgerSnapshot.exists
                ? {
                    ...
                    balance_after: pickNumber(reversalLedgerData, 'balance_after') ??
        ...
        return {
            ...
            cancelled_by_app_user_id: currentCancelledBy ?? params.cancelledByAppUserId,
            cancellation_reason: currentReason ?? params.cancellationReason,
            replacement_sale_id: currentReplacement ?? params.replacementSaleId,
...
async function permanentlyDeleteCustomer(req, customerId) {
    ...
    const authoritativeTombstonePayload = (0, sync_backend_js_1.buildSyncTombstonePayload)(tombstoneId, merchantId, 'customer', customerId, deletedAt);
    return {
        merchant_id: merchantId,
        customer_id: customerId,
...
function customerFirestoreDependencyQueries(merchantId, customerId) {
    const businessRef = businessDocumentRef(merchantId);
    return [
        'sales',
        'redemptions',
...
async function applyCustomerFirestoreHardDelete(merchantId, customerId, tombstonePayload) {
    return admin.firestore().runTransaction(async (transaction) => {
        const customerRef = businessCustomerRef(merchantId, customerId);
        const tombstoneRef = businessSyncTombstoneRef(merchantId, tombstonePayload.id);
        const dependencies = customerFirestoreDependencyQueries(merchantId, customerId);
        const [customerSnapshot, existingTombstoneSnapshot, ...dependencySnapshots] = await Promise.all([
            transaction.get(customerRef),
            transaction.get(tombstoneRef),
        ...
        if (customerSnapshot.exists &&
            ...
            throw new CustomerCoreError(409, 'customer_delete_requires_archive', 'Customer must be archived before permanent deletion.', {
                merchant_id: merchantId,
                customer_id: customerId,
            });
        }
        ...
        const reverseLinkSnapshot = reverseLinkRef
            ...
            : null;
        const reverseLinkCustomerId = reverseLinkSnapshot?.exists === true
        ...
        const existingTombstone = existingTombstoneSnapshot.exists
            ...
            : {};
        const deletedAt = pickNumber(existingTombstone, 'deleted_at') ??
