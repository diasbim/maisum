...
exports.matchKeyDocId = matchKeyDocId;
exports.fnv1a = fnv1a;
/* ----------------------------------------------------------------- domain */
/**
...
function normalizePhone(raw, countryCode) {
    ...
    if (subscriber.length < 8 || subscriber.length > 15)
        return null;
    return `+${subscriber}`;
}
/* ------------------------------------------------------------------- name */
...
function normalizeSocialUrl(raw) {
    ...
    // word "company".
    const handle = host.endsWith('linkedin.com') && segments.length >= 2
 ...
 */
function buildMatchKeys(identity, phoneCountryCode) {
    const keys = [];
    const seen = new Set();
    ...
    if (provider !== '' && orgId !== '')
        push('PROVIDER_ORG', `${provider}/${orgId}`);
