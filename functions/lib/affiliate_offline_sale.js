...
exports.planOfflineReferralSale = planOfflineReferralSale;
exports.commitOfflineReferralSale = commitOfflineReferralSale;
...
function honouredDiscount(input) {
    if (!input.offlineBenefitApplied || input.appliedBenefit === null)
        return 0;
    if (input.appliedBenefit.type === 'POINTS')
        return 0;
...
function eventWrite(merchantId, eventId, event) {
    return {
        kind: 'create',
        path: affiliate_sale_commit_js_1.referralPaths.event(merchantId, eventId),
        data: {
            id: eventId,
...
function fraudSignalWrite(merchantId, signalId, signal) {
    return {
        kind: 'merge',
        path: affiliate_sale_commit_js_1.referralPaths.fraudSignal(merchantId, signalId),
        data: {
            id: signalId,
            merchant_id: merchantId,
 ...
 */
function planOfflineReferralSale(facts, input, config) {
    ...
    const customerPhoneHash = (0, affiliate_engine_js_1.referralPhoneHash)(input.customerPhoneE164);
    const attributionKey = affiliate_engine_js_1.referralIdempotencyKeys.attribution(merchantId, customerPhoneHash);
    ...
    const validation = (0, affiliate_engine_js_1.validateReferral)(snapshot, {
        ...
        linkStatus: facts.link === null
            ...
            : (str(facts.link, 'status') ?? '').toUpperCase() === 'ACTIVE'
                ? 'ACTIVE'
                : 'INACTIVE',
        affiliatePhoneHash: (0, affiliate_engine_js_1.referralPhoneHash)(str(facts.affiliate, 'phone_e164', 'phone')),
        customerPhoneHash,
        customerIsNew,
    ...
    const loyalty = (0, affiliate_sale_commit_js_1.loyaltyRuleFrom)(facts.business);
    const saleBase = {
        id: saleId,
        merchant_id: merchantId,
    ...
    for (const item of input.items) {
        ...
        itemWrites.push({
            kind: 'create',
            path: affiliate_sale_commit_js_1.referralPaths.saleItem(merchantId, item.id),
            data: {
                id: item.id,
                merchant_id: merchantId,
                sale_id: saleId,
    ...
    const appliedMonetaryBenefit = input.offlineBenefitApplied &&
    ...
    const netAmount = monetaryApplied
        ...
        : input.grossAmount;
    const points = (0, affiliate_sale_commit_js_1.calculateSalePoints)(netAmount, loyalty.pointsPerMzn);
    ...
    writes.push({ kind: 'create', path: affiliate_sale_commit_js_1.referralPaths.sale(merchantId, saleId), data: saleData });
    ...
    const pointsCredited = !monetaryApplied &&
        isPointsBenefit &&
    ...
    if (pointsCredited) {
        writes.push({
            kind: 'create',
            path: affiliate_sale_commit_js_1.referralPaths.ledgerEntry(merchantId, (0, affiliate_sale_commit_js_1.referralBonusLedgerEntryId)(saleId)),
            data: {
                id: (0, affiliate_sale_commit_js_1.referralBonusLedgerEntryId)(saleId),
                merchant_id: merchantId,
                customer_id: input.customerId,
                entry_type: affiliate_sale_commit_js_1.REFERRAL_BONUS_ENTRY_TYPE,
                source_type: affiliate_sale_commit_js_1.REFERRAL_BONUS_SOURCE_TYPE,
                source_id: saleId,
    ...
    if (acquires) {
        attributionData = {
            ...
            created_at: facts.existingAttributionDoc === null
                ? input.now
        ...
        writes.push({
            ...
            path: affiliate_sale_commit_js_1.referralPaths.attribution(merchantId, attributionId),
        ...
        writes.push({
            kind: 'merge',
            path: affiliate_sale_commit_js_1.referralPaths.code(merchantId, code.codeId),
        ...
        if (plannedReward !== null && facts.existingRewardDoc === null) {
            rewardData = {
                ...
                idempotency_key: affiliate_engine_js_1.referralIdempotencyKeys.reward(attributionId, plannedReward.type),
                approved_by: null,
            ...
            writes.push({
                kind: 'create',
                path: affiliate_sale_commit_js_1.referralPaths.reward(merchantId, rewardId),
                data: rewardData,
            });
        }
        ...
        const attributedEventId = affiliate_engine_js_1.affiliateIds.event(merchantId, 'REFERRAL_ATTRIBUTED', saleId);
        if (!facts.existingEventIds.has(attributedEventId)) {
            const write = eventWrite(merchantId, attributedEventId, {
                ...
                metadata: {
                    ...
                    clock_skew_exceeded: clockSkewMs > affiliate_engine_js_1.CLOCK_SKEW_SIGNAL_MS,
                },
            });
            writes.push(write);
            events.push(write.data);
        }
        const outboxId = affiliate_engine_js_1.affiliateIds.outbox(merchantId, 'affiliate_new_customer', saleId);
        if (!facts.existingOutboxIds.has(outboxId)) {
            writes.push({
                kind: 'create',
                path: affiliate_sale_commit_js_1.referralPaths.outbox(merchantId, outboxId),
                data: {
                    id: outboxId,
                    merchant_id: merchantId,
                    affiliate_id: code.affiliateId,
        ...
        if (rewardSummary !== null) {
            const rewardEventId = affiliate_engine_js_1.affiliateIds.event(merchantId, 'AFFILIATE_REWARD_CREATED', saleId);
...
function planRefusal(params) {
    ...
    const netAmount = benefitKept
        ...
        : input.grossAmount;
    const points = (0, affiliate_sale_commit_js_1.calculateSalePoints)(netAmount, params.loyalty.pointsPerMzn);
    ...
    if (affiliateId !== null && codeId !== null && facts.existingAttributionDoc === null) {
        ...
        writes.push({
            kind: 'create',
            path: affiliate_sale_commit_js_1.referralPaths.attribution(merchantId, params.attributionId),
            data: attributionData,
        });
    }
    const rejectionEventId = affiliate_engine_js_1.affiliateIds.event(merchantId, 'REFERRAL_REJECTED', params.idempotencyKey);
    if (!facts.existingEventIds.has(rejectionEventId)) {
        const write = eventWrite(merchantId, rejectionEventId, {
            ...
            metadata: {
                ...
                clock_skew_exceeded: params.clockSkewMs > affiliate_engine_js_1.CLOCK_SKEW_SIGNAL_MS,
            },
        });
        writes.push(write);
        events.push(write.data);
    ...
    writes.push(fraudSignalWrite(merchantId, affiliate_engine_js_1.affiliateIds.fraudSignal(merchantId, 'OFFLINE_CODE_REJECTED', saleId), {
        affiliateId,
        customerId: input.customerId,
        saleId,
...
function replaySummary(facts, input, saleId, attributionId, idempotencyKey, extra) {
    const sale = facts.existingSale ?? {};
    const benefitType = str(sale, 'referral_benefit_type');
    ...
    return {
        ...
        referral: {
            ...
            attribution: facts.existingAttributionDoc === null
                ? null
...
async function commitOfflineReferralSale(gateway, input, loadConfig) {
    return gateway.runTransaction(async (transaction) => {
        const facts = await (0, affiliate_sale_commit_js_1.readReferralSaleFacts)(transaction, input);
        const plan = planOfflineReferralSale(facts, input, loadConfig(facts.business));
        for (const operation of plan.writes) {
            if (operation.kind === 'create')
                transaction.create(operation.path, operation.data);
            else
                transaction.merge(operation.path, operation.data);
        }
        switch (plan.status) {
            case 'committed':
