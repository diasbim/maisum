 ...
 * is deliberate.
 */
export const PROSPECT_PIPELINE = [
  'RAW',
  'QUALIFIED',
  'SCORED',
  'ENRICHED',
...
export type ProspectPipelineStatus = (typeof PROSPECT_PIPELINE)[number];
 ...
 */
export const PROSPECT_TERMINAL = [
  'NOT_A_FIT',
  'NO_CONTACT',
  'DO_NOT_CONTACT',
  'OPTED_OUT',
...
export type ProspectTerminalStatus = (typeof PROSPECT_TERMINAL)[number];
...
export function isProspectStatus(value: unknown): value is ProspectStatus {
  return (
    typeof value === 'string' &&
    (PROSPECT_STATUS as readonly string[]).includes(value)
  );
}

export function isTerminalStatus(status: ProspectStatus): boolean {
  return (PROSPECT_TERMINAL as readonly string[]).includes(status);
}
 ...
 * never have sent.
 */
export function blocksOutreach(status: ProspectStatus): boolean {
...
export function canTransition(
  ...
  const fromIndex = (PROSPECT_PIPELINE as readonly string[]).indexOf(from);
  const toIndex = (PROSPECT_PIPELINE as readonly string[]).indexOf(to);
 ...
 * stored.
 */
export const CLAIM_TYPE = ['FACT', 'INFERENCE', 'UNKNOWN'] as const;
...
export type ProspectSource = (typeof PROSPECT_SOURCE)[number];
...
export const OUTREACH_CHANNEL = ['WHATSAPP', 'EMAIL', 'SMS', 'LINKEDIN'] as const;
 ...
 */
export function isDecisionMakerSeniority(seniority: Seniority): boolean {
  return (
    seniority === 'OWNER' ||
    seniority === 'FOUNDER' ||
    seniority === 'C_LEVEL' ||
...
export function isReachableEmail(status: EmailStatus): boolean {
 ...
 */
export const PROVIDER_OPERATION = [
  'SEARCH_BUSINESSES',
  'ENRICH_COMPANY',
...
export type ProviderOperation = (typeof PROVIDER_OPERATION)[number];
 ...
 */
export const FALLBACK_ERROR_CODES: readonly ProviderErrorCode[] = [
  'UNAVAILABLE',
  'TIMEOUT',
  'RATE_LIMITED',
  'INSUFFICIENT_CREDITS',
...
export const JOB_TYPE = ['DISCOVERY', 'BULK_ENRICHMENT'] as const;
export type JobType = (typeof JOB_TYPE)[number];

/** States a sweep may pick up. Terminal rows are never queried again. */
export const CLAIMABLE_JOB_STATUSES: readonly JobStatus[] = ['QUEUED', 'RUNNING'];
