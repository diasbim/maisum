...
import {
  logCustomerRedemptionEvent,
  type CustomerRedemptionEvent,
...
import {
  ...
  listEntitlements as listEntitlementsFromFirestore,
  listMerchants as listMerchantsFromFirestore,
  listPlans as listPlansFromFirestore,
  listStaff as listStaffFromFirestore,
  merchantExists as merchantExistsInFirestore,
  setPlanFeature as setPlanFeatureInFirestore,
  setStaffStatus as setStaffStatusInFirestore,
  setSubscriptionStatus as setSubscriptionStatusInFirestore,
  SUBSCRIPTION_STATUSES,
  upsertEntitlement as upsertEntitlementInFirestore,
  upsertPlan as upsertPlanInFirestore,
...
import {
  OPERATION_COST_USD,
  resolveProspectingFlags,
  type ProspectingSettings,
} from './prospecting_config.js';
import {
  consumeProspectingRateLimit,
  createJob as createProspectingJob,
  cancelJob as cancelProspectingJob,
  driveJob as driveProspectingJob,
  firestoreStore as prospectingFirestoreStore,
  getJob as getProspectingJob,
  listClaimableJobs as listClaimableProspectingJobs,
} from './prospecting_firestore.js';
import { resolveLlm as resolveProspectingLlm } from './prospecting_llm.js';
import type { PipelineDeps } from './prospecting_pipeline.js';
import { AisaProvider } from './prospecting_provider_aisa.js';
import { ApolloProvider } from './prospecting_provider_apollo.js';
import { PlacesProvider } from './prospecting_provider_places.js';
import { TemplateOutreachService } from './prospecting_templates.js';
import {
  FixtureProvider,
  NotConfiguredProvider,
} from './prospecting_provider_fixtures.js';
import { registerProspectingRoutes } from './prospecting_routes.js';
import {
  appendActivity as appendProspectActivity,
  getCompany as getProspectCompany,
  getProspect,
  latestAnalysis as latestProspectAnalysis,
  listActivities as listProspectActivities,
  listContacts as listProspectContacts,
  listProspects,
  listUsage as listProspectingUsage,
  prospectingRefs,
  readSettings as readProspectingSettings,
...
import {
  recordReferredCustomerReturnInFirestore,
...
import type {
  ...
  SharedLoyaltyLedgerEntryType,
  SharedLoyaltyLedgerSourceType,
} from './sync_backend.js';
...
type AuthedRequest = express.Request & {
  ...
  /**
   * Set only by the auth middleware, and only for a valid `x-admin-key` on a
   * path in ADMIN_API_KEY_PATHS. Never derived inside route handlers, where
   * `req.path` is relative to the router mount point and would silently widen
   * the key's reach.
   */
...
const apolloApiKeySecret = defineSecret('APOLLO_API_KEY');
const anthropicApiKeySecret = defineSecret('ANTHROPIC_API_KEY');
const aisaApiKeySecret = defineSecret('AISA_API_KEY');
...
app.use(async (req, res, next) => {
  ...
  if ((!authHeader || !authHeader.startsWith('Bearer ')) &&
      isAdminPath(req) &&
      isAdminApiKeyPath(req.path) &&
      adminApiKeyMatches(
        process.env.ADMIN_API_KEY,
        pickHeaderString(req.headers['x-admin-key']),
      )) {
 ...
 */
function respondAdminServerError(
...
adminRouter.get('/merchants', async (req, res) => {
  ...
  try {
    ...
    return res.json({
      success: true,
      data: page.items.map(toAdminMerchantSummary),
      paging: {
        limit,
        offset,
...
adminRouter.get('/audit-events', async (req, res) => {
  ...
  try {
    const page = await listAuditEventsFromFirestore({
      merchantId:
        pickQueryString(req.query.merchant_id) ??
        pickQueryString(req.query.merchantId) ??
        undefined,
      targetType:
        pickQueryString(req.query.target_type) ??
    ...
    return res.json({
      success: true,
      data: page.items.map(toAdminAuditEvent),
...
adminRouter.get('/operations/summary', async (_req, res) => {
  try {
    const summary = await getOperationsSummaryFromFirestore();
    return res.json({
      success: true,
      data: toAdminOperationsSummary(summary),
...
adminRouter.get('/merchants/:merchantId', async (req, res) => {
  ...
  try {
    const detail = await getMerchantDetailFromFirestore(merchantId);
    if (!detail) {
      return res
        .status(404)
        .json({ success: false, message: 'Merchant not found' });
    }
    return res.json({ success: true, data: toAdminMerchantDetail(detail) });
  } catch (error) {
...
adminRouter.get('/plans', async (_req, res) => {
  try {
    const plans = await listPlansFromFirestore();
    return res.json({ success: true, data: plans.map(toAdminPlan) });
  } catch (error) {
...
function auditActorFrom(req: AuthedRequest): AuditActor {
  return {
...
adminRouter.post('/merchants/:merchantId/subscription/status', async (req, res) => {
  ...
  try {
    const result = await setSubscriptionStatusInFirestore({
      merchantId,
    ...
    await recordAuditEvent(auditActorFrom(req as unknown as AuthedRequest), {
...
adminRouter.post('/merchants/:merchantId/entitlements', async (req, res) => {
  ...
  try {
    if (!(await merchantExistsInFirestore(merchantId))) {
      return res
        .status(404)
        .json({ success: false, message: 'Merchant not found' });
    }

    const { before, after } = await upsertEntitlementInFirestore({
      merchantId,
      featureKey,
      isEnabled,
...
adminRouter.post('/plans', async (req, res) => {
  ...
  try {
    await upsertPlanInFirestore({ planCode, version, name, isActive });
...
adminRouter.post('/prices', async (req, res) => {
  ...
  try {
    await upsertPlanPriceInFirestore({
      planCode,
      currency,
      amount,
      billingPeriod,
...
adminRouter.post('/plans/:planCode/features', async (req, res) => {
  ...
  try {
    const result = await setPlanFeatureInFirestore({
      planCode,
      featureKey,
      isEnabled,
      limitValue: limitValue ?? null,
...
adminRouter.get('/merchants/:merchantId/entitlements', async (req, res) => {
  ...
  try {
    ...
    const entitlements = await listEntitlementsFromFirestore(merchantId);
    return res.json({
      success: true,
      data: entitlements.map(toAdminEntitlement),
    });
  } catch (error) {
...
adminRouter.get('/access/staff', async (req, res) => {
  ...
  try {
    const page = await listStaffFromFirestore({
      search: pickQueryString(req.query.search) ?? undefined,
      merchantId:
        pickQueryString(req.query.merchant_id) ??
        pickQueryString(req.query.merchantId) ??
    ...
    return res.json({
      success: true,
      data: page.items.map(toAdminStaffUser),
      paging: { limit, offset, has_more: page.hasMore },
      truncated: page.truncated,
    });
  } catch (error) {
...
adminRouter.post('/merchants/:merchantId/staff/:userId/status', async (req, res) => {
  ...
  try {
    ...
    const result = await setStaffStatusInFirestore({ merchantId, userId, status });
    if (!result.ok) {
      ...
      return res.status(409).json({
        success: false,
        message:
          'This account is the only active owner of this business. Deactivate another owner first, or leave one active.',
      });
    }
    ...
    return res.json({
      success: true,
      data: toAdminStaffUser({ ...result.after, id: userId, merchant_id: merchantId }),
    });
  } catch (error) {
  ...
  try {
    ...
    do {
      ...
      for (const user of page.users) {
        if (!hasAdminClaims(user.customClaims)) continue;
        admins.push({
          uid: user.uid,
          ...
          claims: adminClaimNames(user.customClaims),
...
adminRouter.get('/customers/lookup', async (req, res) => {
  ...
  try {
    if (phone) {
      canonicalCustomerId = buildCanonicalCustomerId(
        normalizeMozambiquePhoneToE164(phone),
      );
    } else if (cardUid) {
      const normalizedUid = tryNormalizeNfcCardUid(cardUid);
      if (!normalizedUid) {
        return res
          .status(400)
      ...
      const linked = maybePayloadString(
        snapshotDataRecord(cardSnapshot),
        'canonical_customer_id',
      );
      if (!linked) {
        return res
  ...
  try {
    const [identitySnapshot, linkSnapshot, cardsSnapshot, accountLinkSnapshot] =
      await Promise.all([
    ...
    const businesses = linkSnapshot.docs.map((doc) => {
      ...
      return {
        ...
        business_customer_id:
          maybePayloadString(data, 'business_customer_id', 'customer_id') ?? '',
...
adminRouter.get('/customers/:canonicalCustomerId/ledger', async (req, res) => {
  ...
  try {
    ...
    if (businessCustomerId) {
      query = loyaltyLedgerCollectionRef(merchantId).where(
        'customer_id',
        '==',
        businessCustomerId,
      );
...
adminRouter.get('/nfc-cards', async (req, res) => {
  ...
  try {
    if (cardUid) {
      const normalized = tryNormalizeNfcCardUid(cardUid);
      if (!normalized) {
        return res
          .status(400)
          .json({ success: false, message: 'Invalid card UID.' });
      ...
      return res.json({
        success: true,
        data: [toAdminNfcCard(normalized, snapshotDataRecord(snapshot))],
      });
    }
    ...
    if (canonicalCustomerId) {
      ...
      return res.json({
        ...
        data: snapshot.docs.map((doc) =>
          toAdminNfcCard(doc.id, snapshotDataRecord(doc)),
        ),
      });
    }
...
adminRouter.post('/nfc-cards/:cardUid/revoke', async (req, res) => {
  const normalized = isNonEmptyString(req.params.cardUid)
    ? tryNormalizeNfcCardUid(req.params.cardUid.trim())
    : null;
  if (!normalized) {
...
merchantRouter.post('/recovery-tasks/:taskId/complete', async (req, res) => {
  ...
  try {
    ...
    const task = await completeRecoveryTask(pool, {
...
function prospectingProviders(settings: ProspectingSettings) {
  ...
  const fixtures =
    process.env.PROSPECTING_FIXTURES_ENABLED === 'true'
      ? new FixtureProvider()
      : new NotConfiguredProvider('fixtures');

  const byKey: Record<string, unknown> = { apollo, places, fixtures };
  const ordered = settings.providerPriority
    .map((key) => byKey[key])
    .filter((provider): provider is object => provider !== undefined);

  return {
    discovery: ordered as never,
    personDiscovery: ordered as never,
    personEnrichment: ordered as never,
    ...
    webResearch: [aisa, fixtures] as never,
    ...
    listingDetail: [places] as never,
  };
}

function prospectingPipelineDeps(settings: ProspectingSettings): PipelineDeps {
  return {
    store: prospectingFirestoreStore(settings),
...
registerProspectingRoutes({
  ...
  auditActorFrom,
  respondServerError: respondAdminServerError,
  flags: () => resolveProspectingFlags(process.env),
  readSettings: readProspectingSettings,
  writeSettings: writeProspectingSettings,
  pipelineDeps: prospectingPipelineDeps,

  getProspect,
  getCompany: getProspectCompany,
  listProspects,
  listContacts: listProspectContacts,
  listActivities: listProspectActivities,
  latestAnalysis: latestProspectAnalysis,
  saveAnalysis: saveProspectAnalysis,
  setProspectStatus,
  saveScores: saveProspectScores,
  appendActivity: appendProspectActivity,
  ...
  createJob: createProspectingJob,
  getJob: getProspectingJob,
  cancelJob: cancelProspectingJob,
  ...
  consumeRateLimit: ({ actorId, action }) =>
    consumeProspectingRateLimit({ actorId, action }),
...
app.get('/customer/redemptions/:redemptionId', async (req, res) => {
  try {
    const result = await handleCustomerRedemptionStatusRequest(
      req as unknown as AuthedRequest,
...
app.post('/customer/redemptions/:redemptionId/reissue', async (req, res) => {
  try {
    const result = await handleCustomerRedemptionReissueRequest(
      req as unknown as AuthedRequest,
      req.params.redemptionId,
      requireBodyObject(req.body),
    );
...
app.post('/merchant/redemptions/resolve', async (req, res) => {
  try {
    const result = await handleMerchantRedemptionResolveRequest(
      req as AuthedRequest,
      requireBodyObject(req.body),
    );
    return res.json({ success: true, data: result });
  } catch (error) {
    logCustomerRedemptionRejection(
      'merchant',
      error,
      (req as AuthedRequest).merchantId,
    );
    return respondCustomerCoreError(res, error);
  }
});
...
app.post('/merchant/redemptions/consume', async (req, res) => {
  try {
    const result = await handleMerchantRedemptionConsumeRequest(
      req as AuthedRequest,
      requireBodyObject(req.body),
    );
    return res.json({ success: true, data: result });
...
app.post('/return-bonuses/:id/redeem', async (req, res) => {
  ...
  const redemptionSaleId =
    pickString(payload, 'redemption_sale_id') ?? pickString(payload, 'redemptionSaleId');

  try {
    const bonus = await redeemReturnBonus(pool, {
      merchantId,
      customerId,
      bonusId: id,
      redemptionSaleId,
    ...
    if (error instanceof RetentionEngineError) {
...
app.get('/retention/config', async (req, res) => {
  ...
  try {
    await seedDefaultRetentionRules(pool, merchantId, Date.now());
    const [config, rules] = await Promise.all([
      getReturnBonusConfig(pool, merchantId),
      listRetentionRules(pool, merchantId),
    ]);
    return res.json({
      success: true,
      data: {
...
app.put('/retention/config', async (req, res) => {
  ...
  try {
    ...
    if (returnBonusPatch && typeof returnBonusPatch === 'object') {
      ...
      if (validityHours != null) patch.validityHours = validityHours;
      const minimumPurchaseAmount = pickNumber(returnBonusPatch, 'minimum_purchase_amount');
      if (minimumPurchaseAmount != null) patch.minimumPurchaseAmount = minimumPurchaseAmount;
      await upsertReturnBonusConfig(pool, merchantId, patch, now);
    }
    ...
    switch (entityType) {
        ...
        if (operation === 'delete') {
          if (!isOwnerRequest(authedReq)) {
            throw new CustomerCoreError(
              403,
              'customer_delete_owner_required',
              'Only an owner can permanently delete customers.',
            );
          }
      ...
      case 'sale':
        if (operation === 'delete') {
          throw new CustomerCoreError(
            409,
            'sale_delete_forbidden',
            'Sales are immutable; use operation="cancel" instead of delete.',
          );
          ...
          throw new CustomerCoreError(
            ...
            'Sales are immutable after creation; use operation="cancel" for authoritative changes.',
          );
          ...
          throw new CustomerCoreError(
            400,
            'sale_operation_unsupported',
            'sale only supports operation="create" or operation="cancel".',
          );
        }
        return res.json({ success: true });
      case 'sale_item':
      ...
      case 'usage_event':
        if (operation !== 'create') {
          throw new CustomerCoreError(
            400,
            'sync_usage_event_create_only',
            'usage_event is create-only',
          );
        }
      ...
      case 'sync_tombstone':
        throw new CustomerCoreError(
          400,
          'sync_tombstone_read_only',
          'sync_tombstone is server-managed and read-only',
        );
      case 'return_bonus':
        throw new CustomerCoreError(
          400,
          'return_bonus_read_only',
          'return_bonus is server-managed; use POST /return-bonuses/:id/redeem to redeem it.',
        );
      default:
        throw new CustomerCoreError(404, 'sync_unknown_entity', 'Unknown entity');
    }
 ...
 */
function surveyLinkBaseUrl(): string | null {
...
app.get('/engage/surveys/:surveyId/link', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT id, title, is_active FROM surveys WHERE id = $1 AND merchant_id = $2`,
      [surveyId, merchantId],
    );
    ...
    const now = Date.now();
    const token = createSurveyLinkToken({
      merchantId,
      surveyId,
      customerId,
      issuedAt: now,
    ...
    return res.json({
      ...
      data: {
        ...
        expires_at: now + SURVEY_LINK_TTL_MS,
...
function surveyLinkSecret(): string {
...
/** What the person holding the link is allowed to see about the survey. */
type PublicSurvey = {
...
async function readPublicSurvey(
  link: VerifiedSurveyLink,
): Promise<PublicSurvey | null> {
  const surveyResult = await pool.query(
    `SELECT id, title, description, is_active
       FROM surveys WHERE id = $1 AND merchant_id = $2`,
    [link.surveyId, link.merchantId],
  );
  ...
  if (!survey || survey.is_active === false) return null;
...
publicRouter.get('/surveys/:token', async (req, res) => {
  const link = verifySurveyLinkToken({
...
publicRouter.post('/surveys/:token/responses', async (req, res) => {
  ...
  try {
    ...
    const questions = new Map(
      survey.questions.map((question) => [String(question.id), question]),
    );
    const answers: Array<{ questionId: string; text: string | null; numeric: number | null; bool: boolean | null }> = [];
    ...
    const missing = survey.questions.filter(
      (question) => question.is_required === true && !answered.has(String(question.id)),
    );
    if (missing.length > 0) {
      return res.status(400).json({
        success: false,
    ...
    const mirroredAnswers: Array<Record<string, unknown>> = [];
    ...
    try {
      await mirrorSurveyResponseToFirestore(
        link.merchantId,
        {
          id: responseId,
          merchant_id: link.merchantId,
...
async function mirrorSurveyResponseToFirestore(
  merchantId: string,
  ...
  batch.set(
    businessRef.collection('survey_responses').doc(String(response.id)),
    response,
    { merge: true },
  );
  for (const answer of answers) {
    batch.set(
      businessRef.collection('survey_response_answers').doc(String(answer.id)),
      answer,
      { merge: true },
    );
  }
...
export const api = onRequest(
  {
    cors: allowedOrigins(process.env, runningInEmulator(process.env)),
    invoker: 'public',
  ...
  async (event) => {
    ...
    try {
      ...
      try {
        const customerId = maybePayloadString(afterData, 'customer_id', 'customerId');
        const amount = pickNumber(afterData, 'amount');
        const cancellation = (maybePayloadString(
          afterData,
          'cancellation_status',
          'cancellationStatus',
        ) ?? '').toUpperCase();
        if (customerId && amount != null && amount > 0 && cancellation !== 'CANCELLED') {
          await recordReferredCustomerReturnInFirestore({
            merchantId,
            saleId,
            customerId,
            amount,
 ...
 */
export const prospectingJobOnCreate = onDocumentCreated(
  {
  ...
  async (event) => {
    if (!resolveProspectingFlags(process.env).prospectingEnabled) return;

    const jobId = typeof event.params.jobId === 'string' ? event.params.jobId : '';
    if (jobId === '') return;

    try {
      const settings = await readProspectingSettings();
      const job = await driveProspectingJob({
        jobId,
 ...
 */
export const prospectingJobSweep = onSchedule(
  {
  ...
  async () => {
    ...
    try {
      const settings = await readProspectingSettings();
      const jobs = await listClaimableProspectingJobs(5);

      for (const job of jobs) {
        await driveProspectingJob({
          jobId: job.id,
          deps: prospectingPipelineDeps,
          settings,
...
function resolveAppUserId(decoded: admin.auth.DecodedIdToken): string | undefined {
  const claims = decoded as Record<string, unknown>;
  const fromClaims =
    typeof claims.app_user_id === 'string'
      ? claims.app_user_id
...
function resolveAppUserRole(decoded: admin.auth.DecodedIdToken): string {
  const claims = decoded as Record<string, unknown>;
  const claimRole =
    typeof claims.app_user_role === 'string'
...
function isAdminRequest(req: AuthedRequest): boolean {
  ...
  if (req.adminKeyGranted === true) return true;
...
function isOwnerOrAdminRequest(req: AuthedRequest): boolean {
...
function logCustomerRedemptionRejection(
...
function requireAuthenticatedAppUserId(req: AuthedRequest): string {
  const appUserId = req.appUserId?.trim();
  if (!appUserId) {
    throw new CustomerCoreError(
      403,
      'sync_actor_required',
      'An authenticated app user is required for this sync operation.',
    );
  }
  return appUserId;
}
...
async function handleCustomerSessionRequest(
  ...
  const featureFlags = serializeCustomerFeatureFlags(
    firebaseUid,
    businessLinks.map((link) => link.merchant_id!),
  );
  return {
    actor: 'CUSTOMER',
...
function serializeCustomerFeatureFlags(
  ...
  return {
    ...
    customer_redemption_enabled:
      ...
      (merchantIds == null
        ? isCustomerRedemptionUidAllowed(process.env, firebaseUid)
        : isCustomerRedemptionAvailable(
          process.env,
          firebaseUid,
          merchantIds,
...
function requireCustomerRedemptionUid(firebaseUid: string): void {
  if (!isCustomerRedemptionUidAllowed(process.env, firebaseUid)) {
    throw new CustomerCoreError(
      403,
      'customer_redemption_rollout_disabled',
      'Customer redemption is not enabled for this pilot account.',
    );
  }
}

function requireCustomerRedemptionMerchant(merchantId: string): void {
  if (!isCustomerRedemptionMerchantAllowed(process.env, merchantId)) {
    throw new CustomerCoreError(
      403,
      'customer_redemption_rollout_disabled',
      'Customer redemption is not enabled for this pilot business.',
    );
  }
}
...
function serializeCustomerReward(
  ...
  const expired = isCustomerRewardExpired(rewardData);
  return {
    reward_id: rewardId,
...
async function readCustomerBusiness(
  ...
  const nextReward = activeRewards.find((reward) =>
...
async function readCustomerActivity(
  ...
  const entries = (await Promise.all(locators.map(async (locator) => {
    ...
    const ledgerEntries = ledgerSnapshot.docs.map((document) => ({
      ...
      businessName:
        maybePayloadString(relationship.businessData, 'name', 'business_name') ??
        null,
  ...
  const referenceKey = (merchantId: string, id: string) => `${merchantId}\0${id}`;
  const rewardReferences = new Map<
    string,
    { merchantId: string; rewardId: string }
  >();
  const redemptionReferences = new Map<
    string,
    { merchantId: string; redemptionId: string }
  >();
  for (const entry of entries) {
    if (entry.data.reward_id != null) {
      rewardReferences.set(
        referenceKey(entry.merchantId, entry.data.reward_id),
        { merchantId: entry.merchantId, rewardId: entry.data.reward_id },
      );
    }
    if (
      entry.data.entry_type === 'REDEMPTION' &&
      entry.data.source_id.length > 0
    ) {
      redemptionReferences.set(
        referenceKey(entry.merchantId, entry.data.source_id),
        { merchantId: entry.merchantId, redemptionId: entry.data.source_id },
      );
    }
  }
  const rewardNames = new Map<string, string>();
  const redemptionStatuses = new Map<string, string | null>();
  await Promise.all([
    ...[...rewardReferences.entries()].map(async ([key, reference]) => {
      const rewardSnapshot = await businessRewardsCollectionRef(
        reference.merchantId,
      ).doc(reference.rewardId).get();
      if (!rewardSnapshot.exists) return;
      const name = maybePayloadString(snapshotDataRecord(rewardSnapshot), 'name');
      if (name) rewardNames.set(key, name);
    }),
    ...[...redemptionReferences.entries()].map(async ([key, reference]) => {
      const redemptionSnapshot = await businessRedemptionsCollectionRef(
        reference.merchantId,
      ).doc(reference.redemptionId).get();
      if (!redemptionSnapshot.exists) return;
      const redemptionData = snapshotDataRecord(redemptionSnapshot);
      if (!supportsCustomerRedemptionReissue(redemptionData)) {
        redemptionStatuses.set(key, null);
        return;
      }
      redemptionStatuses.set(
        key,
        customerRedemptionFulfillmentState(
          redemptionData,
          Date.now(),
          CUSTOMER_REDEMPTION_CODE_TTL_MS,
        ),
      );
    }),
  ]);
...
async function handleCustomerProfileRequest(
  ...
  return {
    display_name:
      maybePayloadString(account.accountData, 'display_name', 'displayName') ??
...
async function handleCustomerRedemptionRequest(
  ...
  const replayCandidates = await Promise.all(locators.map(async (locator) => {
    ...
    return {
      ...serializeRedemptionFulfillment(
        relationship.merchantId,
        redemptionId,
        redemption,
      ),
  ...
  if (existingReplays.length === 1) {
    const replay = existingReplays[0] as Record<string, unknown>;
    ...
    logCustomerRedemptionEvent({
      ...
      fulfillmentStatus: requirePayloadString(
        replay,
        'fulfillment_status',
      ),
    });
  ...
  const response: Record<string, unknown> = {
    ...serializeRedemptionFulfillment(
      relationship.merchantId,
      requirePayloadString(result, 'redemption_id'),
      redemption,
    ),
...
function requireCustomerRedemptionCode(
  payload: Record<string, unknown>,
): string {
  ...
  if (!/^r1_[A-Za-z0-9_-]{20,100}$/.test(code)) {
    throw new CustomerCoreError(
      400,
      'customer_redemption_code_invalid',
      'A valid redemption_code is required.',
    );
  }
  return code;
}

function serializeRedemptionFulfillment(
  merchantId: string,
  redemptionId: string,
  ...
  return {
    ...
    redemption_code_expires_at: customerRedemptionCodeExpiresAt(
      redemption,
      CUSTOMER_REDEMPTION_CODE_TTL_MS,
    ),
    fulfillment_status: customerRedemptionFulfillmentState(
      redemption,
      now,
      CUSTOMER_REDEMPTION_CODE_TTL_MS,
    ),
...
async function handleCustomerRedemptionStatusRequest(
  req: AuthedRequest,
  redemptionId: string,
): Promise<Record<string, unknown>> {
  requireCustomerFeature('customerRedemptionEnabled');
  ...
  return {
    ...serializeRedemptionFulfillment(
      match.relationship.merchantId,
      redemptionId,
      match.data,
    ),
    confirmed_points: Math.max(
      0,
      pickNumber(match.relationship.customerData, 'confirmed_points') ?? 0,
    ),
  };
}

async function findCustomerAuthorizedRedemption(
  account: CustomerAccountAccess,
  redemptionId: string,
): Promise<{
  relationship: CustomerBusinessRelationship;
  ...
  const locators = await listCustomerRelationshipLocators(
    account.canonicalCustomerId,
  );
  const matches = await Promise.all(locators.map(async (locator) => {
    const relationship = await requireCustomerBusinessRelationship(
    ...
    const ref = businessRedemptionsCollectionRef(
      relationship.merchantId,
    ).doc(redemptionId);
  ...
  const authorized = matches.filter(
    (match): match is NonNullable<typeof match> => match != null,
  );
  if (authorized.length !== 1) {
    throw new CustomerCoreError(
      authorized.length === 0 ? 404 : 409,
      authorized.length === 0
        ? 'customer_redemption_not_found'
        : 'customer_redemption_ambiguous',
      authorized.length === 0
...
async function handleCustomerRedemptionReissueRequest(
  req: AuthedRequest,
  redemptionId: string,
  payload: Record<string, unknown>,
): Promise<Record<string, unknown>> {
  ...
  if (
    ...
    throw new CustomerCoreError(
      400,
      'customer_redemption_reissue_invalid',
  ...
  const result = await admin.firestore().runTransaction(async (transaction) => {
    ...
    if (!supportsCustomerRedemptionReissue(current)) {
      throw new CustomerCoreError(
        409,
        'customer_redemption_reissue_unsupported',
        'This redemption does not support customer code reissue.',
      );
    }
    const state = customerRedemptionFulfillmentState(
      current,
      now,
      CUSTOMER_REDEMPTION_CODE_TTL_MS,
    );
    if (state === 'CONSUMED') {
      throw new CustomerCoreError(
        409,
        'customer_redemption_already_consumed',
        'Consumed redemptions cannot be reissued.',
      );
    }
    const replay =
    ...
    const updated = {
      ...current,
      redemption_code: `r1_${randomBytes(18).toString('base64url')}`,
  ...
  logCustomerRedemptionEvent({
    event: result.lifecycleEvent as CustomerRedemptionEvent,
    surface: 'customer',
...
async function findMerchantRedemptionByCode(
  merchantId: string,
  ...
  if (snapshot.size !== 1) {
    throw new CustomerCoreError(
      ...
      snapshot.empty
        ...
        : 'customer_redemption_code_ambiguous',
      snapshot.empty
...
async function serializeMerchantRedemption(
  merchantId: string,
  redemptionId: string,
  redemption: Record<string, unknown>,
  ...
  if (!customerId || !rewardId) {
    throw new CustomerCoreError(
      409,
      'customer_redemption_inconsistent',
      'Redemption customer or reward context is missing.',
    );
  }
  const [customerSnapshot, rewardSnapshot, businessSnapshot] =
    await Promise.all([
  ...
  return {
    ...
    reward: {
      reward_id: rewardId,
      name:
        maybePayloadString(snapshotDataRecord(rewardSnapshot), 'name') ??
        'Prémio',
    },
    business_name:
      maybePayloadString(
        snapshotDataRecord(businessSnapshot),
        'name',
        'business_name',
      ) ?? 'Negócio',
    idempotent_replay: idempotentReplay,
...
async function requireMerchantRedemptionAccess(
  req: AuthedRequest,
): Promise<string> {
  requireCustomerFeature('customerRedemptionEnabled');
  if (
...
async function handleMerchantRedemptionResolveRequest(
  req: AuthedRequest,
  payload: Record<string, unknown>,
): Promise<Record<string, unknown>> {
  if (
    ...
    throw new CustomerCoreError(
      400,
      'customer_redemption_code_invalid',
      'Only redemption_code is accepted.',
    );
  }
  const merchantId = await requireMerchantRedemptionAccess(req);
  const redemptionCode = requireCustomerRedemptionCode(payload);
  const document = await findMerchantRedemptionByCode(
    merchantId,
    redemptionCode,
  );
  const response = await serializeMerchantRedemption(
    merchantId,
    document.id,
    snapshotDataRecord(document),
  );
  const fulfillmentStatus = response.fulfillment_status as string;
  logCustomerRedemptionEvent({
    event:
...
async function handleMerchantRedemptionConsumeRequest(
  req: AuthedRequest,
  payload: Record<string, unknown>,
): Promise<Record<string, unknown>> {
  ...
  if (
    ...
    throw new CustomerCoreError(
      400,
      'customer_redemption_consume_invalid',
      'redemption_code and idempotency_key are required.',
    );
  }
  const merchantId = await requireMerchantRedemptionAccess(req);
  const redemptionCode = requireCustomerRedemptionCode(payload);
  ...
  const now = Date.now();
  const transactionResult = await admin.firestore().runTransaction(
    async (transaction) => {
      ...
      if (maybePayloadString(data, 'redemption_code') !== redemptionCode) {
        throw new CustomerCoreError(
          409,
          'customer_redemption_code_changed',
          'Redemption code changed during validation.',
        );
      }
      const state = customerRedemptionFulfillmentState(
        data,
      ...
      if (state === 'CONSUMED') {
        if (
          maybePayloadString(data, 'consumption_idempotency_key') ===
          idempotencyKey
        ) {
        ...
        throw new CustomerCoreError(
          409,
          'customer_redemption_already_consumed',
          'Redemption code was already consumed.',
          {
            merchant_id: merchantId,
      ...
      if (state === 'EXPIRED') {
        throw new CustomerCoreError(
          409,
          'customer_redemption_code_expired',
  ...
  const response = await serializeMerchantRedemption(
    ...
    transactionResult.idempotentReplay,
  );
  logCustomerRedemptionEvent({
    ...
    merchantId,
    redemptionId: document.id,
    fulfillmentStatus: response.fulfillment_status as string,
  });
  return response;
}
...
type LoyaltyLedgerEntryType = SharedLoyaltyLedgerEntryType;
type LoyaltyLedgerEntryRecord = SharedLoyaltyLedgerEntryRecord;
type LoyaltyCustomerProjection = SharedLoyaltyCustomerProjection;
...
function businessSyncTombstoneRef(merchantId: string, tombstoneId: string) {
...
function loyaltyLedgerEntryFromData(
  ...
  const entryType: LoyaltyLedgerEntryType =
    entryTypeRaw === 'SALE_REVERSAL'
      ? 'SALE_REVERSAL'
  ...
  const sourceType: SharedLoyaltyLedgerSourceType =
    sourceTypeRaw === 'sale_cancellation'
      ? 'sale_cancellation'
...
function computeCustomerProjectionFromLedgerEntries(
  ...
  return computeCustomerProjectionFromLoyaltyLedgerEntries(entries);
}

function computeLedgerEntriesWithBalances(
  entries: LoyaltyLedgerEntryRecord[],
): LoyaltyLedgerEntryRecord[] {
  return computeLoyaltyLedgerEntriesWithBalances(entries);
}
...
async function handleAssistedLoyaltyRedemptionRequest(
  ...
  const result = await admin.firestore().runTransaction(async (transaction) => {
    ...
    if (isCustomerRewardExpired(rewardData, now)) {
      throw new CustomerCoreError(
        409,
        'loyalty_reward_expired',
        'Reward has expired.',
        {
          merchant_id: merchantId,
          reward_id: rewardId,
    ...
    const redemptionData: Record<string, unknown> = {
      ...
      fulfillment_status:
        maybePayloadString(existingRedemptionData, 'fulfillment_status') ??
        'PENDING',
      redemption_code_expires_at:
        pickNumber(existingRedemptionData, 'redemption_code_expires_at') ??
...
async function upsertCustomer(
  ...
  if ((customerTombstoneResult.rowCount ?? 0) > 0) {
    throw new CustomerCoreError(
      409,
      'customer_deleted_tombstone_conflict',
      'Customer was permanently deleted and cannot be recreated or updated.',
      {
        merchant_id: merchantId,
        customer_id: id,
  ...
  let archiveMutation = {
    ...
    archivedByAppUserId: null as string | null,
  };
  if (req) {
    try {
      archiveMutation = resolveCustomerArchiveMutation(
  ...
  if (req && archiveMutation.touched) {
    await businessCustomerRef(merchantId, id).set(
      buildCustomerArchiveFirestorePatch(archiveMutation, updatedAt),
      { merge: true },
    );
  }
...
async function upsertSale(
  ...
  try {
    assertValidAuthoritativeSaleCreatePayload(payload);
  } catch (error) {
    throw new CustomerCoreError(
      400,
      'sale_create_cancellation_fields_forbidden',
  ...
  const { createdByAppUserId, updatedByAppUserId } = resolveAuthoritativeSaleAuthorship(
    payload,
    req?.appUserId,
  );
  ...
  if (!customerId || amount == null || points == null) {
    throw new CustomerCoreError(
      400,
      'sale_create_missing_fields',
      'Sale create requires customer_id, amount, and points.',
    );
  }
  ...
  if (existingSale.merchant_id !== merchantId) {
    throw new CustomerCoreError(
      409,
      'sale_create_conflict',
      'Sale id already exists for another merchant.',
      { sale_id: id, merchant_id: merchantId },
    );
  }
  if (
    !isCompatibleRepeatedSaleCreate(
    ...
    throw new CustomerCoreError(
      409,
      'sale_create_conflict',
      'Sale already exists with immutable fields that differ from this create request.',
      { sale_id: id, merchant_id: merchantId },
    );
  }
}
 ...
 */
async function mirrorCompletedRecoveryTaskToFirestore(
  merchantId: string,
  ...
  await admin.firestore().runTransaction(async (transaction) => {
    ...
    if (slot !== null && slotRef !== null) {
      ...
      if (holder === taskId) transaction.delete(slotRef);
    }
  });
}

async function mirrorReturnBonusToFirestore(
  merchantId: string,
  bonus: ReturnBonus,
): Promise<void> {
  await admin
 ...
 */
async function triggerSaleCompletedRetentionRules(
  merchantId: string,
  customerId: string,
  saleId: string,
  saleAmount: number,
  ...
  await seedDefaultRetentionRules(pool, merchantId, now);
  const result = await evaluateSaleCompletedRetentionRules(pool, {
    merchantId,
    customerId,
    saleId,
    saleAmount,
...
async function upsertReward(
  ...
  const name = pickString(payload, 'name');
  const pointsRequired =
    pickNumber(payload, 'points_required') ?? pickNumber(payload, 'pointsRequired');
  const description = pickString(payload, 'description');
...
async function upsertAppointment(
  ...
  const customerId = pickString(payload, 'customer_id') ?? pickString(payload, 'customerId');
  const scheduledDate =
    pickNumber(payload, 'scheduled_date') ?? pickNumber(payload, 'scheduledDate');
...
async function upsertRetentionMetric(
  merchantId: string,
  payload: Record<string, unknown>,
  entityId: string,
): Promise<void> {
  ...
  const averageVisitInterval =
    pickNumber(payload, 'average_visit_interval') ?? pickNumber(payload, 'averageVisitInterval') ?? 0;
...
async function upsertAppUser(
  ...
  const invitedByAppUserId =
    pickString(payload, 'invited_by_app_user_id') ??
    pickString(payload, 'invitedByAppUserId');
  const deactivatedAt =
    pickNumber(payload, 'deactivated_at') ?? pickNumber(payload, 'deactivatedAt');
  const createdAt =
    pickNumber(payload, 'created_at') ?? pickNumber(payload, 'createdAt') ?? Date.now();
  const updatedAt = pickNumber(payload, 'updated_at') ?? Date.now();
  const lastLoginAt =
    pickNumber(payload, 'last_login_at') ?? pickNumber(payload, 'lastLoginAt');
...
async function upsertRecoveryTask(
  ...
  const updatedAt = pickNumber(payload, 'updated_at') ?? Date.now();
  const createdByAppUserId =
    pickString(payload, 'created_by_app_user_id') ??
...
async function upsertSurveyResponse(
  ...
  for (const [index, answer] of answers.entries()) {
    ...
    const questionId =
      pickString(row, 'question_id') ?? pickString(row, 'questionId') ?? '';
    const answerId =
      pickString(row, 'id') ??
...
async function cancelSaleViaSync(
  req: AuthedRequest,
  payload: Record<string, unknown>,
  saleId: string,
): Promise<Record<string, unknown>> {
  ...
  const actorAppUserId = requireAuthenticatedAppUserId(req);
  const now = Date.now();
  let cancellationRequest: SaleCancellationRequest;
  try {
    cancellationRequest = resolveSaleCancellationRequest(
      payload,
      actorAppUserId,
      now,
      saleId,
    ...
    throw new CustomerCoreError(
      400,
      'sale_cancellation_invalid',
      error instanceof Error ? error.message : 'Invalid sale cancellation payload.',
    );
  }
  ...
  let affiliateReversal: Record<string, unknown> = { status: 'not_referred' };
  try {
    affiliateReversal = await reverseReferralSaleInFirestore({
      merchantId,
      saleId,
      cancelledAt: cancellationRequest.cancelledAt,
...
async function applySaleCancellationToLoyaltyState(params: {
  ...
  const result = await admin.firestore().runTransaction(async (transaction) => {
    ...
    const currentSaleData = snapshotDataRecord(currentSaleSnapshot);
    const currentCancellationStatus = normalizeSaleCancellationStatus(
      maybePayloadString(currentSaleData, 'cancellation_status', 'cancellationStatus'),
    );
    if (
      ...
      !isCompatibleRepeatedSaleCancellation(
        {
          ...
          cancellationReason: maybePayloadString(
            currentSaleData,
            'cancellation_reason',
            'cancellationReason',
          ),
          replacementSaleId: maybePayloadString(
            currentSaleData,
            'replacement_sale_id',
            'replacementSaleId',
          ),
    ...
    if (saleLedgerSnapshot.exists) {
      ...
      if (
        ...
        throw new CustomerCoreError(
          409,
          'sale_cancellation_reversal_conflict',
          'Sale cancellation reversal ledger entry conflicts with existing immutable data.',
          {
            merchant_id: params.merchantId,
            sale_id: params.saleId,
      ...
      if (customerId) {
        ...
        const customerSnapshot = await transaction.get(customerRef);
        if (customerSnapshot.exists) {
          await updateCustomerProjectionFromLedger(
            transaction,
            params.merchantId,
            customerId,
    ...
    transaction.set(
      currentSaleSnapshot.ref,
      {
        ...
        replacement_sale_id:
          currentReplacement ?? params.replacementSaleId ?? admin.firestore.FieldValue.delete(),
...
async function resolveSqlCustomerDeleteDependencyChecks(
  client: PoolClient,
) {
  ...
  return buildCustomerDeleteDependencyChecks(
