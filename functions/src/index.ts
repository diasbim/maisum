...
adminRouter.get('/merchants', async (req, res) => {
  ...
  try {
    ...
    return res.json({
      success: true,
      data: result.rows.map(toAdminMerchantSummary),
      paging: toAdminPaging(limit, offset, result.rows.length),
    });
  } catch (error) {
...
adminRouter.get('/audit-events', async (req, res) => {
  ...
  try {
    ...
    return res.json({
      success: true,
      data: result.rows.map(toAdminAuditEvent),
      paging: toAdminPaging(limit, offset, result.rows.length),
    });
  } catch (error) {
...
adminRouter.get('/operations/summary', async (_req, res) => {
  ...
  try {
    ...
    return res.json({
      success: true,
      data: toAdminOperationsSummary(result.rows[0]),
    });
  } catch (error) {
...
adminRouter.get('/merchants/:merchantId', async (req, res) => {
  ...
  try {
    ...
    return res.json({
      success: true,
      data: toAdminMerchantDetail(result.rows[0]),
    });
  } catch (error) {
...
adminRouter.get('/plans', async (_req, res) => {
  ...
  try {
    ...
    return res.json({ success: true, data: result.rows.map(toAdminPlan) });
  } catch (error) {
...
adminRouter.get('/merchants/:merchantId/entitlements', async (req, res) => {
  ...
  try {
    ...
    return res.json({
      success: true,
      data: result.rows.map(toAdminEntitlement),
    });
  } catch (error) {
...
adminRouter.get('/access/staff', async (req, res) => {
  ...
  try {
    ...
    return res.json({
      success: true,
      data: result.rows.map(toAdminStaffUser),
      paging: toAdminPaging(limit, offset, result.rows.length),
    });
  } catch (error) {
...
adminRouter.post('/merchants/backfill', async (req, res) => {
  ...
  const startAfter = maybePayloadString(
    payload,
    'start_after_merchant_id',
    'startAfterMerchantId',
  );
  ...
  try {
    ...
    for (const doc of snapshot.docs) {
      const projection = projectBusinessToMerchant(
        doc.id,
        snapshotDataRecord(doc),
        now,
      );
      ...
      if (projection.status === 'skipped') {
        ...
        results.push({
          ...
          message: SKIP_EXPLANATIONS[projection.reason],
        });
        continue;
      }
      ...
      if (outcome === 'phone_conflict') {
        ...
        results.push({
          ...
          message:
            'Outro negócio já tem este telefone. merchants.phone é único.',
        });
        continue;
      }
 ...
 */
async function upsertProjectedMerchant(
  client: PoolClient,
  row: MerchantRow,
): Promise<'written' | 'phone_conflict'> {
 ...
 */
export const merchantProfilePostgresProjection = onDocumentWritten(
  ...
  async (event) => {
    ...
    const result = projectBusinessToMerchant(
      merchantId,
      snapshotDataRecord(afterSnapshot),
      Date.now(),
    );
