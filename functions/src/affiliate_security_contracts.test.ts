...
const ROOT_COLLECTIONS = ['affiliates', 'affiliate_code_lookup'] as const;
...
function matchBlock(collection: string): string {
  const pattern = new RegExp(
    `match /${collection}/\\{[A-Za-z]+\\}\\s*\\{([\\s\\S]*?)\\n(\\s*)\\}`,
  );
  const found = pattern.exec(RULES);
  assert.ok(found, `firestore.rules has no match block for ${collection}`);
  return found[1];
}

test('every merchant-scoped affiliate collection is denied to clients outright', () => {
  for (const collection of MERCHANT_SCOPED_COLLECTIONS) {
    const block = matchBlock(collection);
    assert.match(
      block,
      /allow read,\s*write:\s*if false;/,
      `${collection} is not denied in firestore.rules`,
    );
    ...
    assert.doesNotMatch(
      block,
      /allow (read|get|list)[^:]*:\s*if (?!false)/,
      `${collection} grants a client read`,
    );
  }
});

test('the global affiliate identity and the code lookup are denied too', () => {
  for (const collection of ROOT_COLLECTIONS) {
    assert.match(
      matchBlock(collection),
      /allow read,\s*write:\s*if false;/,
      `${collection} is not denied in firestore.rules`,
    );
  }
...
test('no affiliate collection is in the readable business allowlist', () => {
  const allowlist = /function isReadableBusinessCollection\(collectionId\) \{([\s\S]*?)\n    \}/
    .exec(RULES);
  assert.ok(allowlist, 'the readable-collection allowlist moved');
  for (const collection of MERCHANT_SCOPED_COLLECTIONS) {
    assert.ok(
      !allowlist[1].includes(`'${collection}'`),
      `${collection} is readable by any member of the business`,
    );
  }
});

test('a client cannot write any referral field onto a sale', () => {
  const writable = /function saleClientWritableFields\(\) \{([\s\S]*?)\n    \}/.exec(RULES);
  const serverOwned = /function saleServerOwnedReferralFields\(\) \{([\s\S]*?)\n    \}/
    .exec(RULES);
  assert.ok(writable && serverOwned, 'the sale field lists moved');
  ...
  assert.ok(serverFields.length > 0);
  for (const field of serverFields) {
    assert.ok(
      !writableFields.has(field),
      `${field} is both server-owned and client-writable`,
    );
  }
  // Both gates are wired in, not merely declared.
  assert.match(RULES, /saleCreateIsValid[\s\S]*?saleCarriesNoReferralClaim\(\)/);
  assert.match(RULES, /saleUpdateIsValid[\s\S]*?saleReferralFieldsUnchanged\(\)/);
});

test('every referral field the commit writes onto a sale is named as server-owned', () => {
  const serverOwned = /function saleServerOwnedReferralFields\(\) \{([\s\S]*?)\n    \}/
    .exec(RULES);
  assert.ok(serverOwned);
  const declared = new Set(
    [...serverOwned[1].matchAll(/'([a-z_]+)'/g)].map((match) => match[1]),
  );
  ...
  for (const source of [COMMIT_TS, OFFLINE_TS]) {
    for (const match of source.matchAll(
      /^\s{4}(gross_amount|referral_[a-z_]+|affiliate_(?:id|code_id)):/gm,
    )) {
      written.add(match[1]);
    }
  }

  assert.ok(written.size >= 8, 'the sale document shape could not be read');
  for (const field of written) {
    assert.ok(
      declared.has(field),
      `${field} is written by the server but not listed in saleServerOwnedReferralFields`,
    );
  }
});
...
test('firestore.indexes.json is valid JSON with the shape the CLI deploys', () => {
  const parsed = JSON.parse(INDEXES_RAW) as {
    indexes: IndexEntry[];
    fieldOverrides?: unknown[];
  };
  assert.ok(Array.isArray(parsed.indexes));
  for (const entry of parsed.indexes) {
    assert.equal(typeof entry.collectionGroup, 'string');
    assert.ok(['COLLECTION', 'COLLECTION_GROUP'].includes(entry.queryScope));
    assert.ok(Array.isArray(entry.fields) && entry.fields.length > 0);
    for (const field of entry.fields) {
      assert.equal(typeof field.fieldPath, 'string');
      assert.ok(
        field.order !== undefined || field.arrayConfig !== undefined,
        `${entry.collectionGroup}.${field.fieldPath} has no order`,
      );
    }
  }
});

function affiliateIndexes(): IndexEntry[] {
  const parsed = JSON.parse(INDEXES_RAW) as { indexes: IndexEntry[] };
  return parsed.indexes.filter((entry) =>
    entry.collectionGroup.startsWith('affiliate'),
  );
}

function hasIndexFor(collection: string, fields: string[]): boolean {
  return affiliateIndexes().some(
    (entry) =>
...
test('every scoped list the store runs has an index that starts with the business', () => {
  ...
  for (const collection of [
    ...
    assert.ok(
      hasIndexFor(collection, ['merchant_id']),
      `${collection} has no merchant-scoped index`,
    );
  }
  // The `extra` filter the routes pass is always affiliate_id.
  assert.match(STORE_TS, /query\.where\(extra\.field, '==', extra\.value\)/);
  for (const collection of [
    'affiliate_codes',
    'affiliate_attributions',
    'affiliate_rewards',
    ...
    assert.ok(
      hasIndexFor(collection, ['merchant_id', 'affiliate_id']),
      `${collection} has no merchant + affiliate index`,
    );
  }
});

test('the outbox sweep query is indexed the way it is written', () => {
  // status IN (...) + next_attempt_at <= now, ordered by next_attempt_at.
  assert.match(OUTBOX_FS_TS, /\.where\('status', 'in'/);
  assert.match(OUTBOX_FS_TS, /\.where\('next_attempt_at', '<=', input\.now\)/);
  assert.match(OUTBOX_FS_TS, /\.orderBy\('next_attempt_at', 'asc'\)/);
  assert.ok(
    hasIndexFor('affiliate_outbox', ['status', 'next_attempt_at']),
    'the outbox sweep would need an index Firestore does not have',
  );
});

test('the only index that can cross businesses is the outbox sweep, which no client can reach', () => {
  const crossBusiness = affiliateIndexes().filter(
  ...
  assert.deepEqual(
    crossBusiness.map((entry) => entry.collectionGroup),
    ['affiliate_outbox'],
  );
  assert.match(OUTBOX_FS_TS, /db\(\)\.collectionGroup\('affiliate_outbox'\)/);
  // And the rules deny that collection to every client, so the index is not a
  // way around merchant isolation.
  assert.match(matchBlock('affiliate_outbox'), /allow read,\s*write:\s*if false;/);
  // No merchant-scoped read is left without the business as its first filter.
  for (const entry of affiliateIndexes()) {
    if (entry.queryScope === 'COLLECTION_GROUP') continue;
    if (entry.collectionGroup === 'affiliate_outbox') continue;
    assert.equal(
      entry.fields[0]?.fieldPath,
      'merchant_id',
      `${entry.collectionGroup} is indexed without the business first`,
    );
  }
});
...
function created(sql: string, kind: 'TABLE' | 'INDEX'): string[] {
  const pattern =
    kind === 'TABLE'
...
function dropped(sql: string, kind: 'TABLE' | 'INDEX'): string[] {
  const pattern = new RegExp(`DROP ${kind} IF EXISTS ([a-z_]+)`, 'g');
  return [...sql.matchAll(pattern)].map((match) => match[1]);
}

test('both migration scripts declare one transaction boundary', () => {
  for (const [name, sql] of [
    ['up', UP_SQL],
    ['down', DOWN_SQL],
  ] as const) {
    assert.match(sql, /^BEGIN;/, `${name} does not open a transaction`);
    assert.match(sql.trimEnd(), /COMMIT;$/, `${name} does not commit`);
  }
});

test('the down script structurally names every table the up script creates', () => {
  const createdTables = created(UP_SQL, 'TABLE');
  const droppedTables = new Set(dropped(DOWN_SQL, 'TABLE'));

  assert.ok(createdTables.length >= 9, 'the up script creates fewer tables than the plan');
  for (const table of createdTables) {
    assert.ok(droppedTables.has(table), `${table} survives the rollback`);
  }
  // And nothing else: a rollback that drops `sales` is not a rollback.
  for (const table of droppedTables) {
    assert.ok(
      createdTables.includes(table),
      `the down script drops ${table}, which it did not create`,
    );
  }
});

test('the down script structurally names every legacy column and index added by up', () => {
  const addedColumns = [...UP_SQL.matchAll(/ADD COLUMN IF NOT EXISTS ([a-z_]+)/g)].map(
    (match) => match[1],
  );
  const droppedColumns = new Set(
    [...DOWN_SQL.matchAll(/DROP COLUMN IF EXISTS ([a-z_]+)/g)].map((match) => match[1]),
  );
  assert.ok(addedColumns.length > 0, 'the up script alters no existing table');
  for (const column of addedColumns) {
    assert.ok(droppedColumns.has(column), `${column} survives the rollback`);
  }
  ...
  const legacyIndexes = [...UP_SQL.matchAll(
    /CREATE (?:UNIQUE )?INDEX IF NOT EXISTS ([a-z_]+)\s*\n?\s*ON ([a-z_]+)/g,
  )]
    .filter((match) => !affiliateTables.has(match[2]))
    .map((match) => match[1]);
  const droppedIndexes = new Set(dropped(DOWN_SQL, 'INDEX'));
  for (const index of legacyIndexes) {
    assert.ok(droppedIndexes.has(index), `${index} is left behind on a legacy table`);
  }
});

test('the rollback declares dependency-safe reverse table order', () => {
  const createdTables = created(UP_SQL, 'TABLE');
  ...
  assert.deepEqual(
    droppedTables,
    expected,
    'the rollback drops tables in an order a foreign key could block',
  );
  ...
  assert.ok(lastIndexDrop < firstTableDrop, 'an index is dropped after its table');
});

test('the up script uses idempotent DDL guards', () => {
  for (const statement of statements(UP_SQL)) {
    ...
    if (statement.startsWith('ALTER TABLE')) {
      assert.match(statement, /ALTER TABLE IF EXISTS/);
      for (const line of statement.split('\n')) {
        if (line.includes('ADD COLUMN')) assert.match(line, /ADD COLUMN IF NOT EXISTS/);
      }
    }
  }
});

test('the down script uses idempotent DDL guards', () => {
  for (const statement of statements(DOWN_SQL)) {
    if (statement.startsWith('DROP')) assert.match(statement, /IF EXISTS/);
    if (statement.startsWith('ALTER TABLE')) {
      assert.match(statement, /ALTER TABLE IF EXISTS/);
      for (const line of statement.split('\n')) {
        if (line.includes('DROP COLUMN')) assert.match(line, /DROP COLUMN IF EXISTS/);
      }
    }
  }
});

test('the bootstrap schema and the migration create the same affiliate tables', () => {
  const fromMigration = new Set(created(UP_SQL, 'TABLE'));
  const fromSchema = new Set(
    created(SCHEMA_SQL, 'TABLE').filter((table) => table.startsWith('affiliate')),
  );
  for (const table of fromMigration) {
    assert.ok(
      fromSchema.has(table),
      `${table} exists only in the migration, so a fresh bootstrap lacks it`,
    );
  }
});

test('the uniqueness the feature depends on is enforced by the database, not by hope', () => {
  // One live attribution per customer per business, whatever a race does.
  assert.match(
    UP_SQL,
    /CREATE UNIQUE INDEX IF NOT EXISTS idx_affiliate_attributions_non_rejected_customer\s*\n\s*ON affiliate_attributions\(merchant_id, customer_id\)\s*\n\s*WHERE status <> 'REJECTED';/,
  );
  // One reward per attribution per type: the duplicate-reward conflict rule.
  assert.match(
    UP_SQL,
    /CREATE UNIQUE INDEX IF NOT EXISTS idx_affiliate_rewards_attribution_type\s*\n\s*ON affiliate_rewards\(merchant_id, attribution_id, reward_type\);/,
  );
  ...
  assert.match(
    UP_SQL,
    /CREATE TABLE IF NOT EXISTS affiliate_code_lookup \(\s*\n\s*normalized_code TEXT PRIMARY KEY,/,
  );
  // One affiliate identity per phone.
  assert.match(
    UP_SQL,
    /CREATE UNIQUE INDEX IF NOT EXISTS idx_affiliates_normalized_phone\s*\n\s*ON affiliates\(normalized_phone\);/,
  );
  // One code per affiliate per business.
  assert.match(
    UP_SQL,
    /CREATE UNIQUE INDEX IF NOT EXISTS idx_affiliate_codes_scope\s*\n\s*ON affiliate_codes\(/,
  );
  ...
  for (const name of [
    ...
    assert.ok(SCHEMA_SQL.includes(name), `${name} is missing from schema.sql`);
  }
});

test('the seed carries only obviously synthetic phone numbers', () => {
  ...
  assert.ok(phones.length > 0, 'the seed has no affiliates in it');
  for (const national of phones) {
    ...
    assert.ok(
      /^(\d)\1*$/.test(body) || /^0{4,}\d$/.test(body),
      `+258${national} does not look like a reserved test number`,
    );
  }
});

test('the seed is confined to a demonstrably fake business', () => {
  // Nothing in it may write into a business that could exist in production.
  assert.doesNotMatch(SEED_SQL, /DROP |TRUNCATE |DELETE FROM (?!affiliate)/);
  assert.match(SEED_SQL, /ON CONFLICT/, 'the seed is not re-runnable');
  assert.match(SEED_SQL, /merchant-affiliates-dev/);
  assert.match(SEED_SQL, /MaisUm Afiliados Demo/);
  assert.match(SEED_SQL, /merchant-affiliates-secondary/);
});

test('the ordinary sale trigger invokes the real referred-return command', () => {
  const start = INDEX_TS.indexOf(
    'export const loyaltyLedgerSaleOnSaleWrite = onDocumentWritten',
  );
  assert.ok(start > 0, 'the ordinary sale trigger is gone');
  const end = INDEX_TS.indexOf('\nexport const ', start + 1);
  const trigger = INDEX_TS.slice(start, end === -1 ? INDEX_TS.length : end);
  assert.match(trigger, /recordReferredCustomerReturnInFirestore\(\{/);
  assert.match(trigger, /merchantId,/);
  assert.match(trigger, /saleId,/);
  assert.match(trigger, /customerId,/);
  assert.match(trigger, /cancellation !== 'CANCELLED'/);
});

/* =================================================== where the routes are hung */

test('the referral routes are mounted on the routers that authenticate', () => {
  const registration = /registerAffiliateRoutes\(\{([\s\S]*?)\n\}\);/.exec(INDEX_TS);
  assert.ok(registration, 'registerAffiliateRoutes is no longer called from index.ts');
  assert.match(registration[1], /merchantRouter,/);
  assert.match(registration[1], /adminRouter,/);
  // The resolver and the owner predicate are handed in, not reimplemented.
  assert.match(registration[1], /requireBusiness,/);
  assert.match(registration[1], /isOwnerOrAdminRequest,/);
});

test('the admin router refuses anybody who is not an internal admin, before any handler', () => {
  const guard = /const adminRouter = express\.Router\(\);\s*adminRouter\.use\(\(req, res, next\) => \{([\s\S]*?)\n\}\);/
    .exec(INDEX_TS);
  assert.ok(guard, 'the admin router guard moved or was removed');
  assert.match(guard[1], /isAdminRequest\(req as AuthedRequest\)/);
  assert.match(guard[1], /status\(403\)/);
  // And it is that router, gated, which is mounted at /admin.
  assert.match(INDEX_TS, /app\.use\('\/admin', adminRouter\)/);
});

/** The source of every route registered on one router, with its handler. */
function handlersOn(router: 'merchantRouter' | 'adminRouter'): string[] {
  const bodies: string[] = [];
...
test('no merchant route takes the business from the request instead of resolving it', () => {
  const merchantHandlers = handlersOn('merchantRouter');
  assert.ok(merchantHandlers.length > 0, 'no merchant affiliate routes are registered');
  ...
  for (const handler of merchantHandlers) {
    ...
    assert.doesNotMatch(
      handler,
      /req\.(body|query|params)\.merchant_?[Ii]d|body\.merchant_?[Ii]d/,
      `a merchant handler reads a business id from the request:\n${handler.slice(0, 200)}`,
    );
    assert.match(
      handler,
      /requireBusiness\(/,
      `a merchant handler does not resolve its business:\n${handler.slice(0, 200)}`,
    );
  }
});

test('an admin route that names a business checks that the business exists', () => {
  const adminHandlers = handlersOn('adminRouter');
  assert.ok(adminHandlers.length > 0, 'no admin affiliate routes are registered');

  for (const handler of adminHandlers) {
    if (!/req\.params\.merchantId/.test(handler)) continue;
    ...
    assert.match(
      ...
      `an admin handler trusts a business id it was handed:\n${handler.slice(0, 200)}`,
    );
  }
});

test('nothing that decides money is read off the request', () => {
  ...
  for (const field of [
    ...
    assert.doesNotMatch(
      ROUTES_TS,
      new RegExp(`req\\.body\\.${field}|body\\.${field}\\b`),
      `${field} is taken from the caller`,
    );
  }
  // The commit takes a gross amount and a code, and derives the rest.
  assert.match(COMMIT_TS, /const benefit = calculateBenefit\(code, input\.grossAmount\)/);
  assert.match(COMMIT_TS, /loyaltyRuleFrom\(facts\.business\)/);
});

/* ================================================= the customer's own ledger */

test("an affiliate's reward is never written into a customer's loyalty ledger", () => {
  ...
  assert.ok(!STORE_TS.includes('loyalty_ledger'), 'the reward store writes a ledger entry');
  assert.ok(!ROUTES_TS.includes('loyalty_ledger'), 'a reward route writes a ledger entry');
  ...
  for (const [name, source] of [
    ...
    assert.ok(writes.length > 0, `${name} no longer writes the POINTS benefit`);
    for (const write of writes) {
      assert.match(
        write[1],
        /referralBonusLedgerEntryId\(saleId\)/,
        `${name} writes a ledger entry that is not the referral bonus`,
      );
    }
  }
  assert.match(COMMIT_TS, /REFERRAL_BONUS_ENTRY_TYPE = 'REFERRAL_BONUS'/);
});

test('a reward decision goes through the shared state machine, and PAID is terminal', () => {
  ...
  assert.match(ROUTES_TS, /transitionReward\(/);
  assert.doesNotMatch(
    ROUTES_TS,
    /status === 'PENDING' \?|=== 'APPROVED' \?/,
    'a route decides a reward status by hand',
  );
  ...
  assert.match(contracts, /PAID: \[\],/);
});

/* ===================================================================== logs */
test('no affiliate source logs a phone number without masking it', () => {
  for (const [name, source] of [
    ...
    for (const line of source.split('\n')) {
      ...
      assert.doesNotMatch(
        line,
        /phone(?!_masked|Masked|_hash|Hash)/,
        `${name} logs a phone: ${line.trim()}`,
      );
    }
  }
});

test('no affiliate source logs a token or an authorization header', () => {
  for (const source of [
    ROUTES_TS,
    STORE_TS,
    OUTBOX_FS_TS,
    ...
    for (const line of source.split('\n')) {
      if (!/console\.(log|info|warn|error)/.test(line)) continue;
      assert.doesNotMatch(line, /token|authorization|bearer/i, line.trim());
    }
  }
});
