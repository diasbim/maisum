...
export const GENERATED_FILE = 'plan_policy.generated.ts';
...
export function appDomainDir(repoRoot: string): string {
  return path.join(repoRoot, 'lib', 'features', 'subscription', 'domain');
}
 ...
 * look deliberate.
 */
export function readAppPolicy(domainDir: string): AppPolicy {
  const read = (file: string) =>
    readFileSync(path.join(domainDir, file), 'utf8').replace(/\r\n/g, '\n');
...
type FeatureKeyTable = {
  /** Dart constant name -> stored string, e.g. whatsappAutomation -> whatsapp_automation. */
  values: Map<string, string>;
  /** The stored strings in the order `FeatureKeys.all` declares them. */
  order: string[];
};

function parseFeatureKeys(source: string): FeatureKeyTable {
...
function parsePlanCatalog(
  catalog: string,
  planEnum: string,
  featureKeys: FeatureKeyTable,
): PlanPolicy[] {
...
function parsePlanSwitch(source: string, getter: string): Map<string, string> {
  const block = new RegExp(`String get ${getter} => switch \\(this\\) \\{([\\s\\S]*?)\\};`).exec(
    source,
  );
  if (!block) {
...
export function renderPlanPolicy(policy: AppPolicy): string {
  const lines: string[] = [];
  ...
  lines.push(
    `export type PlanCode = ${policy.plans.map((plan) => `'${plan.code}'`).join(' | ')};`,
  );
  lines.push('');
