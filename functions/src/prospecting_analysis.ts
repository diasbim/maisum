...
import type { ScoreResult } from './prospecting_scoring.js';
 ...
 * data changed.
 */
export function dataHash(input: LeadAnalysisInput): string {
  const material = {
    ...
    contacts: input.contacts
      .map((contact) =>
        [contact.first_name, contact.job_title, contact.seniority, contact.email_status].join('|'),
      )
      .sort(),
 ...
 * is asking any more.
 */
export function cacheHit(stored: CacheKey | null, wanted: CacheKey): boolean {
  if (stored === null) return false;
  return (
    stored.dataHash === wanted.dataHash &&
...
export type LeadAnalysisInput = {
  ...
  webResearch: CompanyResearchResult | null;
  digitalPresence: {
    hasWebsite: boolean;
    socialProfiles: readonly string[];
    hasWhatsApp: boolean;
  };
  /** The deterministic score, given to the model as context, not as a target. */
  score: ScoreResult;
};
...
export type EvidenceItem = {
  claim: string;
  type: Exclude<ClaimType, 'UNKNOWN'>;
  source: string;
};
...
export class AnalysisError extends Error {
  readonly code: 'INVALID_SCHEMA' | 'NOT_CONFIGURED' | 'UNAVAILABLE';
 ...
 * be obeyed.
 */
export function fence(value: string | null, maxChars = 800): string {
 ...
 */
export function buildAnalysisPrompt(input: LeadAnalysisInput): string {
  ...
  const findingLines =
    webResearch === null || webResearch.findings.length === 0
      ? '(nenhuma pesquisa feita)'
      : webResearch.findings
          .map(
            (finding) =>
              `- [${finding.type}] ${fence(finding.claim, 300)} (fonte: ${fence(finding.source, 200)})`,
          )
          .join('\n');
  ...
  return [
    ...
    findingLines,
    '',
    `Desconhecidos já identificados: ${
      webResearch === null || webResearch.unknowns.length === 0
        ? '(nenhum)'
...
export function isCheckableSource(source: string): boolean {
 ...
 */
export function parseAnalysis(raw: unknown): LeadAnalysis {
  ...
  const evidence: EvidenceItem[] = Array.isArray(record.evidence)
    ? record.evidence
        .flatMap((entry) => {
          ...
          if (claim === '' || !isCheckableSource(source)) return [];
          const type = String(item.type ?? '').toUpperCase();
          if (type !== 'FACT' && type !== 'INFERENCE') return [];
          return [{ claim, type: type as EvidenceItem['type'], source }];
        })
        .slice(0, MAX_EVIDENCE)
    : [];

  const unknowns: string[] = Array.isArray(record.unknowns)
    ? record.unknowns
  ...
  const recommendedChannel = (OUTREACH_CHANNEL as readonly string[]).includes(channelRaw)
 ...
 */
export const SCORE_DIVERGENCE_THRESHOLD = 15;

export function scoreDivergence(
  analysis: LeadAnalysis,
  score: ScoreResult,
): { delta: number; notable: boolean } {
  const delta = analysis.fitScore - score.total;
  return { delta, notable: Math.abs(delta) >= SCORE_DIVERGENCE_THRESHOLD };
}
 ...
 * The model, as a port.
 *
 * One method, taking two strings and returning one. Everything that makes a
 * particular model a particular model — the endpoint, the key, the token
 * budget, the retries — lives behind this and never leaks into the analysis
 * logic.
 */
...
export class LeadAnalysisService {
  ...
  cacheKeyFor(input: LeadAnalysisInput): CacheKey {
    return {
   ...
   */
  async analyze(input: {
    lead: LeadAnalysisInput;
    stored: { key: CacheKey; analysis: LeadAnalysis } | null;
    force?: boolean;
    ...
    try {
      ...
      throw new AnalysisError(
        'UNAVAILABLE',
        error instanceof Error ? error.message : 'modelo indisponível',
      );
    }
...
export type OutreachDraft = {
  channel: OutreachChannel;
  locale: string;
  /** Null for the channels that have no subject. */
  subject: string | null;
  body: string;
  /** What the message was addressed to. Never invented. */
  addressedTo: string | null;
  /** True when the draft had to be shortened to the channel's limit. */
  truncated: boolean;
};
...
export class OutreachService {
  ...
  async generate(request: OutreachRequest): Promise<OutreachDraft> {
 ...
 */
export function parseOutreach(raw: unknown, request: OutreachRequest): OutreachDraft {
  let value: unknown = raw;
  if (typeof value === 'string') {
    const text = value.trim().replace(/^```(?:json)?/i, '').replace(/```$/, '');
    try {
  ...
  const subject =
    request.channel === 'EMAIL' ? sanitizeText(record.subject, 120) || null : null;
...
export function availableChannels(
  ...
  if (hasWhatsApp) available.push('WHATSAPP');

  const hasVerifiedEmail =
    contacts.some((contact) => contact.email !== null && contact.email_status === 'VERIFIED') ||
    company.email !== null;
  if (hasVerifiedEmail) available.push('EMAIL');

  if (hasWhatsApp) available.push('SMS');
