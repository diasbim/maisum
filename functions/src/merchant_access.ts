...
const SINGULAR_CLAIMS = ['merchant_id', 'merchantId'] as const;
const LIST_CLAIMS = ['merchant_ids', 'merchantIds'] as const;

/** Business fields that name the owning user. */
const OWNER_FIELDS = ['owner_user_id', 'firebase_uid'] as const;
 ...
 * business to show.
 */
export function merchantIdsFromClaims(claims: unknown): string[] {
  ...
  for (const key of SINGULAR_CLAIMS) add(record[key]);
 ...
 */
export function businessGrantsAccess(
 ...
 * and run a business.
 */
export function withMerchantClaim(
  claims: unknown,
  merchantId: string,
  grant: boolean,
...
export function canAccessBusiness(
  ...
  if (merchantIdsFromClaims(claims).includes(merchantId)) return true;
  if (merchantId === user.uid) return true;
