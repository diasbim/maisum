 ...
 */
export const AISA_ESTIMATED_COST_USD = 0.012;
 ...
 */
export function buildResearchPrompt(input: CompanyResearchInput): string {
  const fence = (value: string | null, max = 200): string => {
...
export function sanitizeClaim(value: unknown, maxChars = 300): string {
...
export function findingsFrom(
  ...
  const allowed = new Set(
    citations.map((entry) => citationKey(entry)).filter((entry): entry is string => entry !== null),
  );
  ...
  for (const entry of raw) {
    ...
    if (type !== 'FACT' && type !== 'INFERENCE') continue;
...
export class AisaProvider implements WebResearchProvider {
  readonly key = AISA_KEY;
  private readonly options: AisaOptions;
  ...
  constructor(options: AisaOptions) {
  ...
  estimatedCostUsd(): number {
    return this.options.estimatedCostUsd ?? AISA_ESTIMATED_COST_USD;
  }

  async researchCompany(input: CompanyResearchInput): Promise<CompanyResearchResult> {
    const operation: ProviderOperation = 'RESEARCH_COMPANY';
    ...
    try {
      response = await fetchImpl(`${baseUrl}/perplexity/sonar`, {
        ...
        body: JSON.stringify({
          model: this.options.model ?? AISA_MODEL,
    ...
    const social = Array.isArray(parsed.social_profiles)
      ? socialFrom(
          parsed.social_profiles.filter((entry): entry is string => typeof entry === 'string'),
        )
      : [];
 ...
 */
export function priceFrom(headers: Headers | undefined): number | null {
 ...
 */
export function wasCharged(code: ProviderErrorCode): boolean {
  return code !== 'NOT_CONFIGURED' && code !== 'RATE_LIMITED';
}

/** The company's own host, as Sonar's domain filter wants it. */
function domainFilter(website: string): string[] {
  try {
...
function messageContent(choices: unknown): string | null {
...
function citationList(record: Record<string, unknown>): string[] {
  const urls: string[] = [];
...
function socialFrom(urls: readonly string[]): string[] {
...
export function parseJsonBlock(content: string): Record<string, unknown> | null {
  ...
  if (start >= 0 && end > start) attempts.push(withoutFence.slice(start, end + 1));
