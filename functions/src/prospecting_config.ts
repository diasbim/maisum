 ...
 *
 * Nothing in the scoring engine, the budget guard or the qualifier carries a
 * literal. They all read a config object, and this file holds the defaults for
 * it. Two reasons, and the second is the one that matters:
 *
 *   A weight that lives in a table can be changed by argument rather than by
 *   deploy, and the argument is about the table, not about a line of code
 *   buried in a scoring function.
 *
 *   A pure function over an explicit config is testable at every setting. The
 *   scoring tests run the same engine over a config with one criterion
 *   disabled, which is how "this point is worth five" is actually checked
 *   rather than asserted about a hard-coded sum.
 *
 * The stored settings document overlays this; `resolveSettings` merges. A
 * field absent from the document takes the default, so adding one here does
...
/**
 * The business types worth selling MaisUm to, and what is true about each.
 *
 * `businessType` is not a new vocabulary. It is the id the product already
 * uses: `BusinessProfile` in `lib/features/business_profile/domain/` defines
 * these, onboarding writes one onto `businesses.business_type`, and
 * `index.ts` reads them for retention windows. A prospecting module with its
 * own industry enum would guarantee that a lead which converts never matches
 * the business it became — the join that answers "did prospecting work?"
 * would silently return nothing.
 *
 * `recurringModel` and `multipleServices` are properties of the trade, not of
 * the individual shop, so they belong here rather than being inferred per
 * lead. A barbershop sells the same haircut to the same person every month:
 * that is a fact about barbershops, and the scoring engine may rely on it.
 * Anything that varies shop to shop — whether this salon runs promotions,
 * whether it has a website — is evidence, and is never read from this table.
 */
export type IcpIndustry = {
  /** Matches `businesses.business_type`. */
  businessType: string;
  label: string;
  /** 1 is the beachhead, 2 the widening. Tier drives the business-fit point. */
  tier: 1 | 2;
  /** The same customer is expected back on a cycle. */
  recurringModel: boolean;
  /** More than one thing is sold, so a loyalty catalogue has something to hold. */
  multipleServices: boolean;
  /** Words that identify this trade in a provider search or a page title. */
  keywords: readonly string[];
};

export const ICP_INDUSTRIES: readonly IcpIndustry[] = [
  {
...
export function findIcpIndustry(
  businessType: string | null,
): IcpIndustry | null {
  if (businessType === null) return null;
  const normalized = businessType.trim().toLowerCase();
  return (
    ICP_INDUSTRIES.find((entry) => entry.businessType === normalized) ?? null
  );
}

/**
 * Where MaisUm is selling right now.
 *
 * Data, not code: the country is a field on the geography, the city list is a
 * list, and adding Beira or Nampula is an edit to a settings document rather
 * than a deploy. The engine matches on the normalised city name and falls back
 * to the province, so a lead in "Matola-Rio" still counts as target geography
 * without the list having to enumerate every suburb.
 */
export type TargetGeography = {
  country: string;
  countryCode: string;
  /** E.164 country calling code, used to normalise a local phone number. */
  phoneCountryCode: string;
  provinces: readonly string[];
  cities: readonly string[];
};

export const DEFAULT_GEOGRAPHY: TargetGeography = {
  country: 'Moçambique',
  countryCode: 'MZ',
...
/**
 * What each criterion is worth, and what the dimension caps at.
 *
 * The caps are not derived from the criteria: `businessFit.max` is 40 while
 * its criteria sum to exactly 40 today, and a later criterion added without
 * raising the cap simply competes for the same 40 points. That is deliberate.
 * A dimension whose maximum moved would change every stored score's meaning,
 * and the bands below would quietly start meaning something else.
 */
export type ScoringConfig = {
  businessFit: {
    max: number;
    targetIndustry: number;
    recurringCustomerModel: number;
    employeeCountInRange: number;
    targetGeography: number;
    /** The band that earns `employeeCountInRange`, inclusive. */
    employeeRange: { min: number; max: number };
    /** What a public rating of at least `minRating` is worth. */
  ...
  digitalPresence: {
    ...
    strongPresence: number;
    /** Distinct social profiles that make a presence "strong". */
    strongPresenceProfiles: number;
    /** A published timetable, which is a profile somebody maintains. */
    openingHours: number;
    photos: number;
  };
  retentionPotential: {
    ...
    promotions: number;
    loyaltyUseCase: number;
    /** Awarded once per threshold in `reviewThresholds` that is met. */
    reviewVolume: number;
    /** Review counts that evidence customer traffic, low to high. */
    reviewThresholds: readonly number[];
  };
  commercialOpportunity: {
    max: number;
    decisionMakerIdentified: number;
    reachableContact: number;
    growthSignal: number;
    /** The business is trading, rather than closed or moved. */
    operational: number;
  };
  /** Ordered high to low; the first whose `min` is met wins. */
  bands: readonly { band: ScoreBand; min: number }[];
};
...
export const SCORE_BAND = ['PRIORITY', 'GOOD', 'NURTURE', 'LOW_FIT'] as const;
...
export function bandFor(score: number, config: ScoringConfig): ScoreBand {
 ...
 */
export const OPERATION_COST_USD: Record<ProviderOperation, number> = {
  /**
 ...
 * shown beside it.
 */
export const ESTIMATES_VERIFIED = false;
 ...
 * upper unit.
 */
export const ENRICHMENT_UNIT_COST_USD = OPERATION_COST_USD.FETCH_LISTING_DETAILS;

/* -------------------------------------------------------------- settings */

/**
 * What an admin may change from `Definições → Prospeção`.
 *
 * Stored in Firestore rather than in environment variables, because §11 asks
 * for them to be editable from the UI and an env var is not — changing one
 * means a deploy, and a budget cap that takes a deploy to lower is not a
...
export const DEFAULT_SETTINGS: ProspectingSettings = {
  monthlyBudgetUsd: 50,
  dailyBudgetUsd: 5,
...
export type CompanySizeKey = (typeof COMPANY_SIZE_BANDS)[number]['key'];

export function findSizeBand(key: string) {
  return COMPANY_SIZE_BANDS.find((band) => band.key === key) ?? null;
}
 ...
 */
export const DEFAULT_OUTREACH_LOCALE = 'pt-MZ';
 ...
 */
export const OUTREACH_MAX_CHARS: Record<OutreachChannel, number> = {
  WHATSAPP: 600,
  SMS: 300,
  EMAIL: 1400,
  LINKEDIN: 900,
...
/**
 * The module's flags, read from the environment.
 *
 * Environment booleans rather than a settings document, matching
 * `customer_feature_flags.ts` — these decide whether a surface exists at all,
 * which is a deployment question, while the budgets above are an operating
 * question. Off means the endpoints answer "disabled" rather than 404, so the
 * portal can explain instead of looking broken.
 */
export type ProspectingFlagEnvironment = Record<string, string | undefined>;

export type ProspectingFlags = {
...
export function resolveProspectingFlags(
  environment: ProspectingFlagEnvironment,
): ProspectingFlags {
